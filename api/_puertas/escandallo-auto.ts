...
//    en borrador con confianza, para confirmación humana en una pantalla.
//
// LEY-ANTIFALSOS-01: si la suma de líneas no cuadra con el total de cabecera,
// NO se insertan líneas a medias. Si la IA no está segura de un ingrediente
// del conteo, la línea queda sin vincular y con confianza baja: decide Rubén.
//
// Coste: cada factura reprocesada es 1 llamada de visión de pago. El botón de
// la UI procesa lotes pequeños bajo demanda: el gasto siempre lo dispara Rubén.
...
async function estado(res: VercelResponse) {
  ...
  const [pend, borr, alertas, estr] = await Promise.all([
    ...
    try {
      ...
      if (lineas === null) {
        ...
        if (Math.abs(diff) > TOLERANCIA_EUR) {
          estadoF = 'sin_detalle_lineas' // LEY-ANTIFALSOS: no cuadra → no se cargan líneas
          ...
          const rows = lineas.map(l => ({
            ...
            origen: 'ocr_reproceso', // el trigger de BBDD vincula/pre-crea solo
          }))
...
async function leerConteo(req: VercelRequest, res: VercelResponse) {
  ...
  if (!inventario_id || !imagen_base64) return res.status(400).json({ error: 'Faltan inventario_id o imagen_base64' })
  const apiKey = process.env.ANTHROPIC_API_KEY
...
Reglas: transcribe fielmente cada fila con cantidad anotada; NO inventes filas; si una cantidad es ilegible, omite la fila; unidades típicas: Kg., gr., L., ml., Ud.`
  ...
  if (!resp.ok) return res.status(502).json({ error: `Visión HTTP ${resp.status}` })
  const data = await resp.json() as { content?: Array<{ text?: string }> }
  const raw = (data.content || []).map(c => c.text || '').join('').replace(/```json|```/g, '').trim()
  ...
  const { data: ings } = await supabaseAdmin
    .from('ingredientes')
    .select('id, iding, nombre, nombre_base, ud_std')
    .neq('activo', false)
  ...
  const rows = leidas.map(l => {
    ...
    return {
      inventario_id,
      iding: m?.iding ?? null,
      ingrediente_id: m?.id ?? null,
      cantidad: l.cantidad,
      unidad: l.unidad || m?.ud_std || null,
      confianza: m ? m.confianza : 0,
      confirmado: false,
      texto_leido: l.nombre,
    }
  })
...
function matchIngrediente(texto: string, ings: Array<{ id: string; iding: string | null; nombre: string; nombre_base: string | null; ud_std: string | null }>) {
  ...
  return null // ambiguo o desconocido → decide Rubén
}
...
async function confirmarConteo(req: VercelRequest, res: VercelResponse) {
  ...
  if (!inventario_id) return res.status(400).json({ error: 'Falta inventario_id' })
  ...
  await supabaseAdmin.from('inventario_lineas')
