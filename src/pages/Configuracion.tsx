...
const inputCls = 'w-full bg-[#111111] border border-[#2a2a2a] rounded-lg px-3 py-2 text-sm text-[#ffffff] focus:outline-none focus:border-accent'
const inputSmCls = 'w-24 bg-[#111111] border border-[#2a2a2a] rounded px-2 py-1 text-sm text-[#ffffff] text-right'
const btnPrimary = 'px-4 py-2 bg-accent text-black text-sm font-semibold rounded-lg hover:brightness-110 transition'
const btnSecondary = 'px-4 py-2 text-sm text-[#cccccc] border border-[#2a2a2a] rounded-lg hover:text-[#ffffff] hover:border-[#555] transition'
const thCfg = 'px-4 py-3 text-left text-[11px] uppercase tracking-wider text-[#999999] font-semibold bg-[#0a0a0a] border-b border-[#2a2a2a]'
const rowCls = (idx: number) => idx % 2 === 0 ? 'bg-[#1a1a1a]' : 'bg-[#141414]'
const tdCfg = 'px-4 py-2.5 border-b border-[#2a2a2a]'
...
export default function Configuracion() {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-[#ffffff] tracking-tight mb-5">Configuración</h2>
      <div className="flex gap-1 mb-5 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-1 w-fit flex-wrap">
        {SECTIONS.map(s => (
          <button key={s.key} onClick={() => setSection(s.key)}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${section === s.key ? 'bg-accent text-[#111]' : 'text-[#cccccc] hover:text-[#ffffff]'}`}>
            {s.label}
          </button>
        ))}
      </div>
        ...
        <tbody>
          {rows.map((r, idx) => (
            <tr key={r.id} className={rowCls(idx)}>
              <td className={tdCfg + ' text-[#ffffff] font-medium'}>{r.canal}</td>
...
function SecCostes() {
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-6 max-w-lg space-y-4">
      <div>
        <label className="block text-xs text-[#999999] mb-1.5">Coste estructura (%)</label>
        <input type="number" step="0.1" value={estructura} onChange={e => setEstructura(e.target.value)} className={inputCls} />
        <p className="text-[11px] text-[#999999] mt-1">Se aplica sobre PVP neto (sin IVA) en todas las recetas</p>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <span className="text-xs text-[#999999]">{rows.length} proveedores</span>
      ...
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.id} onClick={() => setEdit(r)} className={'cursor-pointer hover:bg-[#0a0a0a] ' + rowCls(idx)}>
                <td className={tdCfg + ' text-[#cccccc]'}>{r.categoria ?? '—'}</td>
                <td className={tdCfg + ' text-[#ffffff] font-mono text-xs font-bold'}>{r.abv}</td>
                <td className={tdCfg + ' text-[#ffffff]'}>{r.nombre_completo}</td>
                <td className={tdCfg + ' text-[#cccccc]'}>{r.marca_asociada ?? '—'}</td>
                <td className={tdCfg + ' text-center'}>
                  <button onClick={e => { e.stopPropagation(); handleDelete(r.id, r.abv) }} className="text-xs text-[#999999] hover:text-[#dc2626] transition">Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <div className="space-y-3">
      ...
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={2} className="p-6 text-center text-[#999999] text-sm">Sin items</td></tr>
            ) : items.map((it, idx) => (
              <tr key={idx} className={rowCls(idx)}>
                <td className={tdCfg + ' text-[#ffffff]'}>{it}</td>
                <td className={tdCfg + ' text-right'}>
                  <button onClick={() => remove(idx)} className="text-xs text-[#999999] hover:text-[#dc2626] transition">×</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#2a2a2a]">
          <h3 className="text-[#ffffff] font-semibold">{title}</h3>
          <button onClick={onClose} className="text-[#999999] hover:text-[#ffffff] text-xl leading-none">×</button>
        </div>
...
function CfgField({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      <label className="block text-xs text-[#999999] mb-1">{label}</label>
...
function Loader() {
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-12 text-center">
      ...
      <p className="text-[#999999] text-sm mt-3">Cargando…</p>
    </div>
  )
}
