    ...
    <div style={{ backgroundColor: BG_OPS, minHeight: '100vh', padding: '1.5rem', fontFamily: FONT.body }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button
          ...
          style={{ backgroundColor: '#e8f442', color: '#111111', border: 'none', borderRadius: 6, padding: '0.5rem 1.25rem', fontFamily: FONT.heading, fontSize: 12, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer' }}
        >
          + Nuevo pedido
        </button>
      </div>
        ...
        <div style={{ backgroundColor: '#141414', border: '1px solid #2a2a2a', borderRadius: 8, padding: '1rem', marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
            {[
              ...
              <div key={f.key}>
                <div style={{ fontSize: 11, color: '#777777', marginBottom: 4 }}>{f.label}</div>
                <input
                  type={f.type}
            ...
            <div>
              <div style={{ fontSize: 11, color: '#777777', marginBottom: 4 }}>Estado</div>
              <select
                value={form.estado}
            ...
            <thead>
              <tr style={{ backgroundColor: '#0a0a0a' }}>
                {['Fecha', 'Proveedor', 'Descripcion', 'Coste', 'Estado'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontFamily: FONT.heading, fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase', color: '#777777', borderBottom: '1px solid #2a2a2a' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {pedidos.length === 0 ? (
                ...
                <tr key={p.id} style={{ backgroundColor: i % 2 === 0 ? BG_OPS : '#141414', borderBottom: '1px solid #2a2a2a' }}>
                  ...
                  <td style={{ padding: '10px 14px', color: '#e8f442' }}>{p.coste !== null ? fmtEur(p.coste) : '—'}</td>
