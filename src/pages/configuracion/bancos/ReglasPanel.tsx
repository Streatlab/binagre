...
interface Regla {
  ...
  categoria_codigo: string | null   // ahora es id PyG ej "2.11.1"
  activa: boolean
  prioridad: number
}
...
export default function ReglasPanel() {
  ...
  async function refetch() {
    const [r, c] = await Promise.all([
      ...
      supabase.from('categorias_pyg').select('id, nombre, nivel, parent_id').eq('activa', true).order('orden').order('id'),
    ])
  ...
  /* Solo nivel 3 (las hojas son las únicas asignables a movimientos) */
  const cats3 = useMemo(() => cats.filter(c => c.nivel === 3), [cats])

  /* Agrupar nivel 3 por su padre nivel 2, y este por su nivel 1 */
  const cats3PorRama = useMemo(() => {
    const ramas: Record<string, { rama: string; items: CatPyg[] }> = {}
    cats3.forEach(c3 => {
      const padre2 = cats.find(x => x.id === c3.parent_id)
      const padre1 = padre2 ? cats.find(x => x.id === padre2.parent_id) : null
      const ramaLabel = padre1 && padre2
        ? `${padre1.id} ${padre1.nombre} · ${padre2.nombre}`
        : padre2
          ? `${padre2.id} ${padre2.nombre}`
          : 'Otros'
      ;(ramas[ramaLabel] = ramas[ramaLabel] || { rama: ramaLabel, items: [] }).items.push(c3)
    })
    return Object.values(ramas).sort((a, b) => a.rama.localeCompare(b.rama))
  }, [cats, cats3])

  /* Filtrar por tipo: ingresos (rama 1.x) vs gastos (rama 2.x y 3.x) */
  const cats3Filtradas = useMemo(() => {
    return cats3PorRama
      .map(({ rama, items }) => ({
        rama,
        items: items.filter(c => fAsigna === 'ingreso' ? c.id.startsWith('1.') : !c.id.startsWith('1.')),
      }))
      .filter(g => g.items.length > 0)
  }, [cats3PorRama, fAsigna])

  const nombrePorId = useMemo(() => {
    const m: Record<string, string> = {}
    cats.forEach(c => { m[c.id] = c.nombre })
    return m
  }, [cats])
                  ...
                  <td onClick={() => open(r)} style={{ ...td, cursor: 'pointer' }}>
                    {r.categoria_codigo ? (
                      <>
                        ...
                        <span>{nombrePorId[r.categoria_codigo] ?? '—'}</span>
                      </>
                    ) : '—'}
                  </td>
      ...
      {(editing || creating) && (
        ...
        >
          ...
          <Field label="Categoría P&G">
            ...
            >
              <option value="">—</option>
              {cats3Filtradas.map(g => (
                <Fragment key={g.rama}>
                  <optgroup label={g.rama}>
                    {g.items.map(c => <option key={c.id} value={c.id}>{c.id} · {c.nombre}</option>)}
                  </optgroup>
                </Fragment>
              ))}
            </select>
