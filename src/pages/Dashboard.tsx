    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[var(--sl-card)] border border-border rounded-xl p-5">
          <p className="text-xs text-[var(--sl-text-secondary)] uppercase tracking-wide mb-4">Bruto ultimas 4 semanas</p>
          {weekBars.length === 0 ? (
            ...
            <div className="flex items-end gap-3 h-40">
              {weekBars.map(bar => (
                <div key={bar.label} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[10px] text-[var(--sl-text-secondary)] tabular-nums">{fmtEur(bar.bruto)}</span>
        ...
        {/* Top 5 days */}
        <div className="bg-[var(--sl-card)] border border-border rounded-xl p-5">
          <p className="text-xs text-[var(--sl-text-secondary)] uppercase tracking-wide mb-4">Top 5 dias con mas ventas</p>
          {topDays.length === 0 ? (
            <p className="text-[var(--sl-text-secondary)] text-sm text-center py-8">Sin datos</p>
          ) : (
                    ...
                    <div className="flex items-center justify-between mb-1" style={{ minWidth: 0 }}>
                      <div className="flex items-center gap-2" style={{ minWidth: 0 }}>
                        <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${
                          i === 0 ? 'bg-accent text-black' : 'bg-[var(--sl-card)]/5 text-[var(--sl-text-secondary)]'
                        }`}>
                          {i + 1}
                        </span>
                        <span className="text-sm text-[var(--sl-text-primary)] truncate">{fmtDate(d.fecha)}</span>
                      </div>
                      <span className="text-sm font-semibold text-[var(--sl-text-primary)] tabular-nums" style={{ flexShrink: 0 }}>{fmtEur(d.bruto)}</span>
                    </div>
                    <div className="h-1 bg-[var(--sl-card)]/5 rounded-full overflow-hidden ml-7">
                      <div
                        className="h-full rounded-full transition-all"
