import { AZUL, BLANCO, CREMA, GRANATE, NAR, ROJO, ROSA, VERDE } from '@/styles/neobrutal'
...
export default function Ocr({ periodoExterno }: { periodoExterno?: { desde: Date; hasta: Date } } = {}) {
  ...
  const cardStyle = (_filtro: FiltroCard, isActive: boolean): React.CSSProperties => ({ background: isActive ? 'var(--neo-bg-2)' : 'var(--sl-card)', border: '3px solid var(--neo-ink)', padding: '16px 16px', cursor: 'pointer', boxShadow: isActive ? '2px 2px 0 var(--neo-shadow-color)' : '4px 4px 0 var(--neo-shadow-color)', transform: isActive ? 'translate(2px, 2px)' : 'none', transition: 'transform 0.08s, box-shadow 0.08s' })
  ...
  return (
    <div style={integrado ? { padding: 0 } : groupStyle(T)}>
      {!integrado && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}><div><h1 style={{ fontFamily: FONT.heading, fontSize: 22, fontWeight: 600, color: GRANATE, textTransform: 'uppercase', letterSpacing: '3px', margin: 0 }}>OCR</h1><p style={{ fontFamily: FONT.body, fontSize: 13, color: 'var(--sl-text-muted)', marginTop: 4, marginBottom: 0 }}>{periodoLabel}</p></div><SelectorFechaUniversal nombreModulo="ocr" defaultOpcion="mes_en_curso" onChange={(desde, hasta, label) => { setFechaDesdeInt(desde); setFechaHastaInt(hasta); setPeriodoLabel(label) }} /></div>
      )}
      ...
      {(tab === 'facturas' || tab === 'otros') && (<>
        ...
        {!cargando && total === 0 && !errorCarga ? (<div style={{ background: 'var(--sl-card)', border: '3px solid var(--neo-ink)', boxShadow: '4px 4px 0 var(--neo-shadow-color)', padding: '48px 28px', textAlign: 'center' }}><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 16, color: 'var(--sl-text-muted)', letterSpacing: 1, marginBottom: 8 }}>{soloDuplicados ? 'No hay posibles duplicados' : emptyLabel}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: 'var(--sl-text-muted)' }}>{soloDuplicados ? 'Nada que revisar' : emptySub}</div></div>) : (
          <div style={{ background: 'var(--sl-card)', border: '3px solid var(--neo-ink)', boxShadow: '4px 4px 0 var(--neo-shadow-color)', overflow: 'hidden' }}>
