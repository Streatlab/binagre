...
export default function Ocr() {
  ...
  // Conteo global de posibles duplicados (toda la base, no solo el periodo). Es
  // un aviso de revisión: si hay, aparece el banner para abrir la bandeja.
  ...
  return (
    <div style={groupStyle(T)}>
      ...
      {tab === 'ventas' && (<div style={{ marginTop: 14 }}><div style={{ marginBottom: 14 }}><BtnSubirSplit label="Subir ventas" accept={ACCEPT_FACTURAS} extensiones={EXT_ACEPTADAS_FACTURAS} preparando={preparando} setPreparando={setPreparando} onArchivos={(r) => { setVerRechazados(false); setModalConfirmarSubida({ archivos: r.aceptados, totalOriginal: r.totalOriginal, rechazados: r.rechazados, expandidosZip: r.expandidosZip, visible: true, fnName: 'ocr-procesar-factura' }) }} /></div><CardFacturasCorreo tipo="ventas" desde={periodoDesdeStr} hasta={periodoHastaStr} onBarrido={() => setRefreshTick(x => x + 1)} /><VentasTab fechaDesde={fechaDesde} fechaHasta={fechaHasta} titulares={titulares} /></div>)}
      {(tab === 'facturas' || tab === 'otros') && (<>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12, marginTop: 14 }}>
          ...
          {tab === 'facturas'
            ? (<CardFacturasCorreo tipo="factura" desde={periodoDesdeStr} hasta={periodoHastaStr} activa={soloCorreo} onClick={onToggleSoloCorreo} onBarrido={() => setRefreshTick(x => x + 1)} />)
            : (<div />)}
        </div>
        {tab === 'facturas' && dupTotal > 0 && (
          <div onClick={onToggleSoloDuplicados} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', marginBottom: 12, background: soloDuplicados ? '#F26B1F18' : '#FFF7ED', border: soloDuplicados ? '1px solid #F26B1F' : '0.5px solid #F26B1F80', borderRadius: 10, cursor: 'pointer' }}>
            <span style={{ fontFamily: 'Oswald, sans-serif', fontSize: 12, letterSpacing: '1px', color: '#F26B1F', fontWeight: 600, textTransform: 'uppercase' }}>⚠ {dupTotal} posibles duplicados</span>
            <span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#7a8090' }}>misma factura, archivo distinto · nada se ha borrado</span>
            <div style={{ flex: 1 }} />
            <span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#B01D23', fontWeight: 500, textDecoration: 'underline' }}>{soloDuplicados ? 'Ver todas' : 'Revisar bandeja'}</span>
          </div>
        )}
        ...
        {soloDuplicados && (<div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#7a8090', marginBottom: 10 }}>Bandeja de posibles duplicados (toda la base, ignora el periodo). Revisa cada par y borra la sobrante con el selector de la izquierda si procede.</div>)}
