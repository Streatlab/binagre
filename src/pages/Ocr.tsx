...
function BtnSubirSplit({ label, accept, extensiones, onArchivos, preparando, setPreparando }: BtnSubirSplitProps) {
  ...
  return (
    <div style={{ display: 'flex', borderRadius: 14, overflow: 'hidden', position: 'relative' }}>
      ...
      <div onDragOver={e => { if (preparando) return; e.preventDefault(); e.stopPropagation(); setOverL(true) }} onDragLeave={e => { e.stopPropagation(); setOverL(false) }} onDrop={e => { if (preparando) return; e.preventDefault(); e.stopPropagation(); setOverL(false); handleFiles(e.dataTransfer.files) }} onClick={handleClickArchivos} style={{ ...halfBase, background: overL ? '#8f1519' : GRANATE, borderRight: '1px solid rgba(255,255,255,0.25)' }}>
      ...
      <div onDragOver={e => { if (preparando) return; e.preventDefault(); e.stopPropagation(); setOverR(true) }} onDragLeave={e => { e.stopPropagation(); setOverR(false) }} onDrop={e => { if (preparando) return; e.preventDefault(); e.stopPropagation(); setOverR(false); handleFiles(e.dataTransfer.files) }} onClick={handleClickCarpetas} style={{ ...halfBase, background: overR ? '#8f1519' : GRANATE }}>
