    ...
    <div style={integrado ? { padding: 0 } : groupStyle(T)}>
      ...
      {(tab === 'facturas' || tab === 'otros') && (<>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 12, marginBottom: 12, marginTop: 14 }}>
          ...
          <div onClick={() => onCambiarFiltroCard('conciliadas')} style={cardStyle('conciliadas', filtroCard === 'conciliadas')}><div style={{ marginBottom: 8 }}><span style={{ fontFamily: 'Oswald, sans-serif', fontSize: 11, fontWeight: 500, letterSpacing: '2px', color: 'var(--sl-text-muted)', textTransform: 'uppercase' }}>Conciliadas</span></div><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 26, fontWeight: 600, lineHeight: 1, letterSpacing: '0.5px', color: '#1D9E75' }}>{agregados ? fmtNumES(agregados.conciliadasCount) : '—'}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: 'var(--sl-text-muted)', marginTop: 4 }}>{agregados ? `${agregados.conciliadasPct}% · ${fmtNumES(agregados.conciliadasImporte, 2)}` : '—'}</div></div>
          <div onClick={() => onCambiarFiltroCard('pendientes')} style={cardStyle('pendientes', filtroCard === 'pendientes')}><div style={{ marginBottom: 8 }}><span style={{ fontFamily: 'Oswald, sans-serif', fontSize: 11, fontWeight: 500, letterSpacing: '2px', color: 'var(--sl-text-muted)', textTransform: 'uppercase' }}>Pendientes</span></div><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 26, fontWeight: 600, lineHeight: 1, letterSpacing: '0.5px', color: '#F26B1F' }}>{agregados ? fmtNumES(agregados.pendientesCount) : '—'}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: 'var(--sl-text-muted)', marginTop: 4 }}>{agregados ? `Faltan datos · ${fmtNumES(agregados.pendientesImporte, 2)}` : '—'}</div></div>
          ...
          {tab === 'facturas' && (
            <div style={cardStyle(null, soloDuplicados)}>
              ...
              <div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 26, fontWeight: 600, lineHeight: 1, letterSpacing: '0.5px', color: dupTotal > 0 ? '#F26B1F' : '#1D9E75' }}>{fmtNumES(dupTotal)}</div>
              ...
              <button onClick={(e) => { e.stopPropagation(); onToggleSoloDuplicados() }} disabled={dupTotal === 0 && !soloDuplicados} style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: 'none', background: (dupTotal === 0 && !soloDuplicados) ? 'var(--sl-border)' : '#B01D23', color: '#fff', fontFamily: 'Oswald, sans-serif', fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: (dupTotal === 0 && !soloDuplicados) ? 'default' : 'pointer', fontWeight: 600 }}>{soloDuplicados ? 'Ver todas' : 'Revisar bandeja'}</button>
            </div>
          )}
        ...
        {!cargando && total === 0 && !errorCarga ? (<div style={{ background: 'var(--sl-card)', border: '3px solid var(--neo-ink)', boxShadow: '4px 4px 0 var(--neo-shadow-color)', padding: '48px 28px', textAlign: 'center' }}><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 16, color: 'var(--sl-text-muted)', letterSpacing: 1, marginBottom: 8 }}>{soloDuplicados ? 'No hay posibles duplicados' : emptyLabel}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: 'var(--sl-text-muted)' }}>{soloDuplicados ? 'Nada que revisar' : emptySub}</div></div>) : (
          <div style={{ background: 'var(--sl-card)', border: '3px solid var(--neo-ink)', boxShadow: '4px 4px 0 var(--neo-shadow-color)', overflow: 'hidden' }}>
            {cargando ? <div style={{ padding: '24px 16px', textAlign: 'center', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: 'var(--sl-text-muted)' }}>Cargando…</div> : (<div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, tableLayout: 'fixed', minWidth: 940, fontFamily: 'Lexend, sans-serif', fontSize: 13 }}><colgroup><col style={{ width: 40 }} /><col style={{ width: 90 }} /><col /><col style={{ width: '14%' }} /><col style={{ width: 110 }} /><col style={{ width: 200 }} /><col style={{ width: 60 }} /><col style={{ width: 130 }} /><col style={{ width: 100 }} /></colgroup><thead><tr><th style={{ padding: '10px 8px', background: 'var(--neo-bg-2)', borderBottom: '3px solid var(--neo-ink)', textAlign: 'center' }}><input type="checkbox" checked={todasSeleccionadas} ref={el => { if (el) el.indeterminate = !todasSeleccionadas && algunaSeleccionada }} onChange={toggleSeleccionTodas} style={{ width: 14, height: 14, cursor: 'pointer', accentColor: '#FF4757' }} /></th>{HEADERS.map(h => <SortableHeader key={h.col} col={h.col} label={h.label} sortIndex={ms.sortIndex(h.col)} sortDir={ms.sortDir(h.col)} onToggle={ms.toggleSort} align={h.align} />)}</tr></thead><tbody>
