...
export default function Ocr({ periodoExterno }: { periodoExterno?: { desde: Date; hasta: Date } } = {}) {
  ...
  const cardStyle = (_filtro: FiltroCard, isActive: boolean): React.CSSProperties => ({ background: isActive ? '#F3D9A8' : '#fff', border: '3px solid #140f08', padding: '16px 16px', cursor: 'pointer', boxShadow: isActive ? '2px 2px 0 #140f08' : '4px 4px 0 #140f08', transform: isActive ? 'translate(2px, 2px)' : 'none', transition: 'transform 0.08s, box-shadow 0.08s' })
    ...
    <div style={integrado ? { padding: 0 } : groupStyle(T)}>
      ...
      {(tab === 'facturas' || tab === 'otros') && (<>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 12, marginBottom: 12, marginTop: 14 }}>
          <div onClick={() => onCambiarFiltroCard(null)} style={cardStyle(null, filtroCard === null && !soloCorreo && !soloDuplicados)}><div style={{ marginBottom: 8 }}><span style={{ fontFamily: 'Oswald, sans-serif', fontSize: 11, fontWeight: 500, letterSpacing: '2px', color: '#7a8090', textTransform: 'uppercase' }}>Total facturas</span></div><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 26, fontWeight: 600, lineHeight: 1, letterSpacing: '0.5px', color: '#111' }}>{agregados ? fmtNumES(agregados.totalCount) : '—'}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: '#7a8090', marginTop: 4 }}>{agregados ? fmtNumES(agregados.totalImporte, 2) : '—'}</div></div>
          <div onClick={() => onCambiarFiltroCard('conciliadas')} style={cardStyle('conciliadas', filtroCard === 'conciliadas')}><div style={{ marginBottom: 8 }}><span style={{ fontFamily: 'Oswald, sans-serif', fontSize: 11, fontWeight: 500, letterSpacing: '2px', color: '#7a8090', textTransform: 'uppercase' }}>Conciliadas</span></div><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 26, fontWeight: 600, lineHeight: 1, letterSpacing: '0.5px', color: '#1D9E75' }}>{agregados ? fmtNumES(agregados.conciliadasCount) : '—'}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: '#7a8090', marginTop: 4 }}>{agregados ? `${agregados.conciliadasPct}% · ${fmtNumES(agregados.conciliadasImporte, 2)}` : '—'}</div></div>
          <div onClick={() => onCambiarFiltroCard('pendientes')} style={cardStyle('pendientes', filtroCard === 'pendientes')}><div style={{ marginBottom: 8 }}><span style={{ fontFamily: 'Oswald, sans-serif', fontSize: 11, fontWeight: 500, letterSpacing: '2px', color: '#7a8090', textTransform: 'uppercase' }}>Pendientes</span></div><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 26, fontWeight: 600, lineHeight: 1, letterSpacing: '0.5px', color: '#F26B1F' }}>{agregados ? fmtNumES(agregados.pendientesCount) : '—'}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 11, color: '#7a8090', marginTop: 4 }}>{agregados ? `Faltan datos · ${fmtNumES(agregados.pendientesImporte, 2)}` : '—'}</div></div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', marginTop: integrado ? 14 : 0 }}><div style={{ flex: 1, minWidth: 240, position: 'relative' }}><input type="text" value={busqueda} onChange={e => onCambiarBusqueda(e.target.value)} placeholder="Buscar contraparte, NIF o número de factura…" style={{ width: '100%', padding: '10px 36px 10px 14px', borderRadius: 0, border: '3px solid #140f08', background: '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#140f08', outline: 'none', boxSizing: 'border-box' }} />{busqueda && <button onClick={() => onCambiarBusqueda('')} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: '#f5f3ef', border: 'none', borderRadius: '50%', width: 22, height: 22, cursor: 'pointer', fontSize: 14, color: '#7a8090', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>}</div><select value={catFiltro} onChange={e => onCambiarCatFiltro(e.target.value)} style={{ padding: '10px 14px', borderRadius: 0, border: '3px solid #140f08', background: '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#140f08', minWidth: 280, cursor: 'pointer' }}><option value="todas">Categorías</option>{categoriasPyg.filter(c => c.nivel === 3).map(c => <option key={c.id} value={c.id}>{c.id} · {c.nombre}</option>)}</select>{integrado && (<button onClick={onToggleSoloDuplicados} title="Posibles duplicados (toda la base)" style={{ padding: '10px 16px', borderRadius: 0, border: soloDuplicados ? '3px solid #140f08' : '3px solid #140f08', background: soloDuplicados ? '#F26B1F15' : '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: soloDuplicados ? '#F26B1F' : (dupTotal > 0 ? '#B01D23' : '#3a4050'), cursor: 'pointer', fontWeight: 500, whiteSpace: 'nowrap' }}>{soloDuplicados ? 'Ver todas' : `Duplicados${dupTotal > 0 ? ` (${dupTotal})` : ''}`}</button>)}<button onClick={onToggleSoloPorCompletar} title="Facturas a las que les falta importe, titular o categoría" style={{ padding: '10px 16px', borderRadius: 0, border: soloPorCompletar ? '3px solid #140f08' : '3px solid #140f08', background: soloPorCompletar ? '#F26B1F15' : '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: soloPorCompletar ? '#F26B1F' : (porCompletarCount > 0 ? '#B01D23' : '#3a4050'), cursor: 'pointer', fontWeight: 500, whiteSpace: 'nowrap' }}>{soloPorCompletar ? 'Ver todas' : `⚠ Por completar${porCompletarCount > 0 ? ` (${porCompletarCount})` : ''}`}</button><ClearSortButton show={ms.showClearButton} onClear={ms.clearSorts} /><button onClick={() => setPausarActualizacion(v => !v)} style={{ padding: '10px 18px', borderRadius: 0, border: '3px solid #140f08', background: pausarActualizacion ? '#F26B1F15' : '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: pausarActualizacion ? '#F26B1F' : '#3a4050', cursor: 'pointer', fontWeight: 500 }}>{pausarActualizacion ? '⏸ Actualización pausada' : 'Pausar actualización'}</button><button onClick={handleExportar} disabled={exportando} style={{ padding: '10px 18px', borderRadius: 0, border: '3px solid #140f08', background: '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#3a4050', cursor: exportando ? 'default' : 'pointer', fontWeight: 500, opacity: exportando ? 0.6 : 1 }}>{exportando ? 'Exportando...' : 'Exportar'}</button></div>
        ...
        {seleccionadas.size > 0 && (<div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', marginBottom: 12, background: '#ffe0ea', border: '3px solid #140f08', boxShadow: '3px 3px 0 #140f08' }}><span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#B01D23', fontWeight: 500 }}>{seleccionadas.size} factura{seleccionadas.size > 1 ? 's' : ''} seleccionada{seleccionadas.size > 1 ? 's' : ''}</span><div style={{ flex: 1 }} />{!confirmarBorrarLote ? (<><button onClick={() => setSeleccionadas(new Set())} style={{ padding: '6px 12px', borderRadius: 0, border: '2px solid #140f08', background: '#fff', color: '#140f08', fontFamily: 'Lexend, sans-serif', fontSize: 12, cursor: 'pointer' }}>Quitar selección</button>{soloDuplicados && (<button onClick={handleNoDuplicado} disabled={marcandoNoDup} style={{ padding: '6px 14px', borderRadius: 0, border: '2px solid #140f08', background: '#fff', color: '#0FB86B', fontFamily: 'Oswald, sans-serif', fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 500, opacity: marcandoNoDup ? 0.6 : 1 }}>{marcandoNoDup ? 'Marcando…' : 'No es duplicado'}</button>)}<button onClick={() => setConfirmarBorrarLote(true)} style={{ padding: '6px 14px', borderRadius: 0, border: '2px solid #140f08', background: '#FF1E27', color: '#fff', fontFamily: 'Oswald, sans-serif', fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 500 }}>Borrar seleccionadas</button></>) : (<><span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#B01D23', fontWeight: 500 }}>¿Seguro? Se borran las facturas, sus asociaciones y los PDFs en Drive.</span><button onClick={() => setConfirmarBorrarLote(false)} disabled={borrandoLote} style={{ padding: '6px 12px', borderRadius: 0, border: '2px solid #140f08', background: '#fff', color: '#140f08', fontFamily: 'Lexend, sans-serif', fontSize: 12, cursor: 'pointer' }}>Cancelar</button><button onClick={handleBorrarLote} disabled={borrandoLote} style={{ padding: '6px 14px', borderRadius: 0, border: '2px solid #140f08', background: '#FF1E27', color: '#fff', fontFamily: 'Oswald, sans-serif', fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 500, opacity: borrandoLote ? 0.6 : 1 }}>{borrandoLote ? 'Borrando…' : 'Sí, borrar'}</button></>)}</div>)}
        {errorCarga && (<div style={{ background: '#ffe0ea', border: '3px solid #140f08', boxShadow: '3px 3px 0 #140f08', borderRadius: 0, padding: '10px 14px', margin: '0 0 12px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#B01D23' }}><span>{errorCarga}</span><button onClick={() => { cargarPagina(); cargarAgregados() }} style={{ background: '#B01D23', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 14px', fontFamily: 'Oswald, sans-serif', fontSize: 11, textTransform: 'uppercase', cursor: 'pointer' }}>Reintentar</button></div>)}
        {!cargando && total === 0 && !errorCarga ? (<div style={{ background: '#fff', border: '3px solid #140f08', boxShadow: '4px 4px 0 #140f08', padding: '48px 28px', textAlign: 'center' }}><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 16, color: '#7a8090', letterSpacing: 1, marginBottom: 8 }}>{soloDuplicados ? 'No hay posibles duplicados' : emptyLabel}</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#7a8090' }}>{soloDuplicados ? 'Nada que revisar' : emptySub}</div></div>) : (
          <div style={{ background: '#fff', border: '3px solid #140f08', boxShadow: '4px 4px 0 #140f08', overflow: 'hidden' }}>
            {cargando ? <div style={{ padding: '24px 16px', textAlign: 'center', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#7a8090' }}>Cargando…</div> : (<div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, tableLayout: 'fixed', minWidth: 940, fontFamily: 'Lexend, sans-serif', fontSize: 13 }}><colgroup><col style={{ width: 40 }} /><col style={{ width: 90 }} /><col /><col style={{ width: '14%' }} /><col style={{ width: 110 }} /><col style={{ width: 200 }} /><col style={{ width: 60 }} /><col style={{ width: 130 }} /><col style={{ width: 100 }} /></colgroup><thead><tr><th style={{ padding: '10px 8px', background: '#F3D9A8', borderBottom: '3px solid #140f08', textAlign: 'center' }}><input type="checkbox" checked={todasSeleccionadas} ref={el => { if (el) el.indeterminate = !todasSeleccionadas && algunaSeleccionada }} onChange={toggleSeleccionTodas} style={{ width: 14, height: 14, cursor: 'pointer', accentColor: '#FF4757' }} /></th>{HEADERS.map(h => <SortableHeader key={h.col} col={h.col} label={h.label} sortIndex={ms.sortIndex(h.col)} sortDir={ms.sortDir(h.col)} onToggle={ms.toggleSort} align={h.align} />)}</tr></thead><tbody>
              {filasVisibles.length === 0 ? (<tr><td colSpan={9} style={{ padding: '32px 16px', textAlign: 'center', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#7a8090' }}>Sin resultados</td></tr>) : filasVisibles.map((f, idx) => {
                const isLast = idx === filasVisibles.length - 1; const tdBase: React.CSSProperties = { padding: '8px 16px', borderBottom: isLast ? 'none' : '1px solid #e6d9bd', verticalAlign: 'middle', lineHeight: 1.4 }; const tdDocBase: React.CSSProperties = { padding: 0, borderBottom: isLast ? 'none' : '1px solid #e6d9bd', verticalAlign: 'middle', textAlign: 'center' }; const catInfo = getBadgeCategoria(f); const estadoDoc = getEstadoDoc(f); const conciliada = estadoDoc === 'conciliada'; const titNombre = titulares.find(t => t.id === f.titular_id)?.nombre?.toLowerCase() ?? ''; const isRuben = titNombre.includes('rubén') || titNombre.includes('ruben'); const isEmilio = titNombre.includes('emilio'); const contraparte = f.proveedor_nombre || '—'; const sel = seleccionadas.has(f.id)
            ...
            {total > 0 && (() => { const desde = (page - 1) * pageSize + 1; const hasta = Math.min(page * pageSize, total); const isFirst = page === 1, isLast2 = page === totalPages; const btnBase: React.CSSProperties = { background: '#fff', border: '2px solid #140f08', borderRadius: 0, padding: '6px 12px', fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#140f08', cursor: 'pointer' }; const btnDis: React.CSSProperties = { ...btnBase, opacity: 0.35, cursor: 'default' }; return (<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: '#F3D9A8', borderTop: '3px solid #140f08' }}><span style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#7a8090' }}>{`Mostrando ${desde.toLocaleString('es-ES')}–${hasta.toLocaleString('es-ES')} de ${total.toLocaleString('es-ES')} facturas`}</span>{totalPages > 1 && (<div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><label style={{ fontFamily: 'Oswald, sans-serif', fontSize: 10, color: '#7a8090', textTransform: 'uppercase' }}>Filas:</label><select value={pageSize} onChange={e => updateUrl({ page: 1, size: Number(e.target.value) as PageSize })} style={{ padding: '6px 10px', borderRadius: 0, border: '2px solid #140f08', background: '#fff', fontFamily: 'Lexend, sans-serif', fontSize: 13 }}>{PAGE_SIZES.map(s => <option key={s} value={s}>{s}</option>)}</select><button style={isFirst ? btnDis : btnBase} disabled={isFirst} onClick={() => !isFirst && updateUrl({ page: 1 })}>Primera</button><button style={isFirst ? btnDis : btnBase} disabled={isFirst} onClick={() => !isFirst && updateUrl({ page: page - 1 })}>‹ Anterior</button><span style={{ ...btnBase, cursor: 'default' }}>{`Página ${page} de ${totalPages}`}</span><button style={isLast2 ? btnDis : btnBase} disabled={isLast2} onClick={() => !isLast2 && updateUrl({ page: page + 1 })}>Siguiente ›</button><button style={isLast2 ? btnDis : btnBase} disabled={isLast2} onClick={() => !isLast2 && updateUrl({ page: totalPages })}>Última</button></div>)}</div>) })()}
          </div>
        )}
      </>)}
      {modalConfirmarSubida.visible && (<div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}><div style={{ background: '#FCEFD6', padding: 28, minWidth: 380, maxWidth: 560, maxHeight: '85vh', border: '4px solid #140f08', boxShadow: '6px 6px 0 #140f08', display: 'flex', flexDirection: 'column' }}><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 14, letterSpacing: '2px', textTransform: 'uppercase', color: '#B01D23', marginBottom: 12 }}>Confirmar subida</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 16, color: '#111', marginBottom: 6 }}>Vas a subir <strong style={{ fontFamily: 'Oswald, sans-serif', fontSize: 20, color: '#1D9E75' }}>{modalConfirmarSubida.archivos.length + modalConfirmarSubida.comprimidos.length}</strong> archivos</div>{modalConfirmarSubida.rechazados.length > 0 && (<><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 14, color: '#E24B4A', marginBottom: 8 }}>Rechazados: <strong>{modalConfirmarSubida.rechazados.length}</strong>{' '}<button onClick={() => setVerRechazados(v => !v)} style={{ background: 'none', border: 'none', color: '#B01D23', fontFamily: 'Lexend, sans-serif', fontSize: 12, cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>{verRechazados ? 'ocultar' : 'ver lista'}</button></div>{verRechazados && (<div style={{ background: '#fff', border: '2px solid #140f08', borderRadius: 0, padding: '10px 12px', maxHeight: 200, overflowY: 'auto', fontFamily: 'Lexend, sans-serif', fontSize: 11, color: '#7a8090', marginBottom: 8 }}>{modalConfirmarSubida.rechazados.map((n, i) => <div key={i} style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', padding: '2px 0' }}>{n}</div>)}</div>)}</>)}<div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 12, color: '#7a8090', marginTop: 8, marginBottom: 18 }}>Se procesarán con OCR y se guardarán en el sistema</div><div style={{ display: 'flex', gap: 10 }}><button onClick={() => { setModalConfirmarSubida({ archivos: [], comprimidos: [], totalOriginal: 0, rechazados: [], expandidosZip: 0, comprimidosServidor: 0, visible: false, fnName: 'ocr-procesar-factura' }); setVerRechazados(false) }} style={{ flex: 1, padding: '12px 14px', borderRadius: 0, border: '3px solid #140f08', background: '#fff', color: '#140f08', fontFamily: 'Oswald, sans-serif', fontSize: 12, letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer' }}>Cancelar</button><button disabled={modalConfirmarSubida.archivos.length + modalConfirmarSubida.comprimidos.length === 0} onClick={() => { const a = [...modalConfirmarSubida.archivos, ...modalConfirmarSubida.comprimidos]; const fn = modalConfirmarSubida.fnName; setModalConfirmarSubida({ archivos: [], comprimidos: [], totalOriginal: 0, rechazados: [], expandidosZip: 0, comprimidosServidor: 0, visible: false, fnName: 'ocr-procesar-factura' }); setVerRechazados(false); procesar(a, fn, null) }} style={{ flex: 1, padding: '12px 14px', borderRadius: 0, border: '3px solid #140f08', boxShadow: '3px 3px 0 #140f08', background: (modalConfirmarSubida.archivos.length + modalConfirmarSubida.comprimidos.length) === 0 ? '#d0c8bc' : '#B01D23', color: '#fff', fontFamily: 'Oswald, sans-serif', fontSize: 12, letterSpacing: '2px', textTransform: 'uppercase', cursor: (modalConfirmarSubida.archivos.length + modalConfirmarSubida.comprimidos.length) === 0 ? 'not-allowed' : 'pointer', fontWeight: 600 }}>Enviar {modalConfirmarSubida.archivos.length + modalConfirmarSubida.comprimidos.length}</button></div></div></div>)}
      {modalTitular.visible && (<div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}><div style={{ background: '#FCEFD6', padding: 28, minWidth: 380, maxWidth: 560, border: '4px solid #140f08', boxShadow: '6px 6px 0 #140f08' }}><div style={{ fontFamily: 'Oswald, sans-serif', fontSize: 14, letterSpacing: '2px', textTransform: 'uppercase', color: '#B01D23', marginBottom: 12 }}>Extracto bancario</div><div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 16, color: '#111', marginBottom: 6 }}>Vas a subir <strong style={{ fontFamily: 'Oswald, sans-serif', fontSize: 20, color: '#1D9E75' }}>{modalTitular.archivos.length + modalTitular.comprimidos.length}</strong> archivos</div>{modalTitular.rechazados.length > 0 && (<div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#E24B4A', marginBottom: 8 }}>Rechazados: <strong>{modalTitular.rechazados.length}</strong></div>)}<div style={{ fontFamily: 'Lexend, sans-serif', fontSize: 13, color: '#111', marginTop: 10, marginBottom: 14 }}>¿De quién es este extracto?</div><div style={{ display: 'flex', gap: 10 }}><button disabled={modalTitular.archivos.length + modalTitular.comprimidos.length === 0} onClick={() => { const a = [...modalTitular.archivos, ...modalTitular.comprimidos]; setModalTitular({ archivos: [], comprimidos: [], totalOriginal: 0, rechazados: [], expandidosZip: 0, comprimidosServidor: 0, visible: false }); procesar(a, 'ocr-procesar-extracto', RUBEN_ID) }} style={{ flex: 1, padding: '12px 14px', borderRadius: 0, border: '3px solid #140f08', boxShadow: '3px 3px 0 #140f08', background: '#FF6A1A', color: '#fff', fontFamily: 'Oswald, sans-serif', fontSize: 12, letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer', opacity: (modalTitular.archivos.length + modalTitular.comprimidos.length) === 0 ? 0.4 : 1 }}>Rubén</button><button disabled={modalTitular.archivos.length + modalTitular.comprimidos.length === 0} onClick={() => { const a = [...modalTitular.archivos, ...modalTitular.comprimidos]; setModalTitular({ archivos: [], comprimidos: [], totalOriginal: 0, rechazados: [], expandidosZip: 0, comprimidosServidor: 0, visible: false }); procesar(a, 'ocr-procesar-extracto', EMILIO_ID) }} style={{ flex: 1, padding: '12px 14px', borderRadius: 0, border: '3px solid #140f08', boxShadow: '3px 3px 0 #140f08', background: '#2D5BFF', color: '#fff', fontFamily: 'Oswald, sans-serif', fontSize: 12, letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer', opacity: (modalTitular.archivos.length + modalTitular.comprimidos.length) === 0 ? 0.4 : 1 }}>Emilio</button></div><button onClick={() => setModalTitular({ archivos: [], comprimidos: [], totalOriginal: 0, rechazados: [], expandidosZip: 0, comprimidosServidor: 0, visible: false })} style={{ marginTop: 14, width: '100%', padding: '8px', background: 'none', border: 'none', color: '#7a8090', fontFamily: 'Lexend, sans-serif', fontSize: 12, cursor: 'pointer' }}>Cancelar</button></div></div>)}
