...
export default function Ocr() {
  ...
  return (
    <div style={groupStyle(T)}>
      ...
      {tab === 'ventas' && (<div style={{ marginTop: 14 }}><div style={{ marginBottom: 14 }}><BtnSubirSplit label="Subir ventas" accept={ACCEPT_FACTURAS} extensiones={EXT_ACEPTADAS_FACTURAS} preparando={preparando} setPreparando={setPreparando} onArchivos={(r) => { setVerRechazados(false); setModalConfirmarSubida({ archivos: r.aceptados, totalOriginal: r.totalOriginal, rechazados: r.rechazados, expandidosZip: r.expandidosZip, visible: true, fnName: 'ocr-procesar-factura' }) }} /></div><CardFacturasCorreo tipo="ventas" desde={periodoDesdeStr} hasta={periodoHastaStr} /><VentasTab fechaDesde={fechaDesde} fechaHasta={fechaHasta} titulares={titulares} /></div>)}
