...
import {
  COLORS, FONT, CARDS, TABS_PILL, LAYOUT,
  kpiBig, kpiMid, kpiSm, lblSm, lblXs, lbl,
...
// Canales visibles: si está en el array se muestra
...
// Margen neto estimado 34% (mismo factor que Panel Global)
...
function buildSemanas(rows: RawDiario[]): SemanaGroup[] {
  ...
  for (const r of rows) {
    ...
    let e=map.get(key); if(!e){e={rows:[],year,week};map.set(key,e)}
    e.rows.push(r)
  }
...
function buildMeses(rows: RawDiario[]): MesGroup[] {
  ...
  for (const [key,mRows] of sorted) {
    const [yStr,mStr]=key.split('-'); const anio=Number(yStr); const mes=Number(mStr)
    const agg=aggregate(mRows); const dias=new Set(mRows.map(r=>r.fecha)).size
    const media_diaria=dias>0?agg.total_bruto/dias:0
    const vs_anterior=prev!==null&&prev>0?((agg.total_bruto-prev)/prev)*100:null
    result.push({anio,mes,dias,...agg,media_diaria,vs_anterior}); prev=agg.total_bruto
  }
...
export default function Facturacion() {
  ...
  // Periodo desde SelectorFechaUniversal
  ...
  // Filtrar por periodo + servicio
  const filteredData = useMemo(()=>{
    const desdeStr=periodoDesde.toISOString().slice(0,10)
    const hastaStr=periodoHasta.toISOString().slice(0,10)
    return allData
      .filter(r=>r.fecha>=desdeStr && r.fecha<=hastaStr)
      .filter(r=>servicioFiltro==='Todos'||r.servicio===servicioFiltro)
  ...
  const canalDropLabel = canalesVisibles.length===5?'Canales':canalesVisibles.length===0?'Sin canales':`${canalesVisibles.length} canales`

  const toggleCanal=(id: CanalId)=>{
    setCanalesVisibles(prev=>{
      if(id==='uber'&&prev.length===5) return prev // no dejar vacío
  ...
  return (
    <div style={{ background: COLORS.bg, padding: LAYOUT.pagePadding }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:18, flexWrap:'wrap', gap:12 }}>
        <div>
          ...
          <span style={{ fontFamily: FONT.body, fontSize:13, color: COLORS.mut, display:'block', marginTop:4 }}>
            {fmtFechaCorta(periodoDesdeStr)} — {fmtFechaCorta(periodoHastaStr)}
          </span>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          {/* Filtro servicio */}
          {['Todos','ALM','CENAS'].map(s=>(
            <button key={s} onClick={()=>setServicioFiltro(s)} style={{
              padding:'6px 14px', borderRadius:8, fontFamily: FONT.heading, fontSize:11,
              fontWeight:500, letterSpacing:'1.5px', textTransform:'uppercase',
              border: servicioFiltro===s?'none':`0.5px solid ${COLORS.brd}`,
              background: servicioFiltro===s?COLORS.redSL:COLORS.card,
              color: servicioFiltro===s?'#fff':COLORS.mut, cursor:'pointer',
            }}>{s}</button>
          ))}
          {/* Dropdown canales — FILTRA DE VERDAD */}
          <div style={{ position:'relative' }} data-drop-canal="canales">
            <button onClick={e=>{e.stopPropagation();setDropCanalOpen(p=>!p)}} style={{
              padding:'6px 12px', borderRadius:8, fontFamily: FONT.heading, fontSize:11,
              letterSpacing:'1.5px', textTransform:'uppercase', border:`0.5px solid ${COLORS.brd}`,
              background: COLORS.card, color: COLORS.mut, cursor:'pointer', display:'flex', alignItems:'center', gap:4,
            }}>
              {canalDropLabel} <span style={{ fontSize:10 }}>▾</span>
            </button>
            {dropCanalOpen && (
              <div style={{ position:'absolute', top:'110%', right:0, background: COLORS.card, border:`0.5px solid ${COLORS.brd}`, borderRadius:10, padding:'6px 0', zIndex:20, minWidth:160, boxShadow:'0 4px 16px rgba(0,0,0,0.08)' }}>
                {ALL_COLS.map(c=>(
                  <label key={c.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 14px', cursor:'pointer', fontFamily: FONT.body, fontSize:13, color: COLORS.text as string ?? COLORS.sec }}>
          ...
          <SelectorFechaUniversal nombreModulo="facturacion" defaultOpcion="mes_en_curso" onChange={(desde,hasta,label)=>{ setPeriodoDesde(desde); setPeriodoHasta(hasta); setPeriodoLabel(label) }} />
          {/* Botón añadir día estilo OCR */}
          <button onClick={()=>setShowAdd(true)} style={{
            display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
            padding:'8px 20px', borderRadius:10, background: COLORS.redSL, color:'#fff', border:'none',
            cursor:'pointer', fontFamily: FONT.heading, fontSize:11, letterSpacing:'1.5px', textTransform:'uppercase', fontWeight:600, gap:2,
          }}>
      ...
      {/* TABS PASTILLA — calco Conciliación */}
      <div style={{ marginBottom: LAYOUT.sectionMargin }}>
        <TabsPastilla
          tabs={TABS_CFG.map(t=>({id:t.key, label:t.label}))}
          activeId={tab}
          onChange={id=>{ setTab(id as Tab); if(id!=='diario') setWeekFilter(null) }}
        />
      </div>
      ...
      {loading ? (
        ...
        <>
          {tab==='diario'  && <TabDiario allData={filteredData} cols={cols} weekFilter={weekFilter} onRefresh={refresh} onEdit={setEditRow} tipoDia={tipoDia} totals={totals} dias={dias} tm={tm} tmNeto={tmNeto} netoEstimado={netoEstimado} mediadiaria={mediadiaria} mediaDiariaNeta={mediaDiariaNeta} />}
          {tab==='semanas' && <TabSemanas allData={filteredData} cols={cols} onDrill={(y,w)=>{setWeekFilter({year:y,week:w});setTab('diario')}} totals={totals} />}
          {tab==='meses'   && <TabMeses allData={filteredData} cols={cols} totals={totals} />}
          {tab==='anual'   && <TabAnual allData={filteredData} />}
        </>
      )}
...
// ─── KPI Cards Panel Global style ─────────────────────────────
interface KpiCardsProps {
  totals: AggRow; dias: number; tm: number; tmNeto: number
  netoEstimado: number; mediadiaria: number; mediaDiariaNeta: number
}

function KpiCards({ totals, dias, tm, tmNeto, netoEstimado, mediadiaria, mediaDiariaNeta }: KpiCardsProps) {
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
      {/* Card 1: Facturación */}
      <div style={CARDS.big}>
        <div style={{ ...lblSm, marginBottom:14 }}>FACTURACIÓN</div>
        <div style={{ display:'flex', alignItems:'baseline', gap:16, marginBottom:8, flexWrap:'wrap' }}>
          <div>
            <div style={{ ...kpiBig, lineHeight:1 }}>{fmtN(Math.round(totals.total_bruto))}</div>
            <div style={{ ...lblXs, marginTop:4 }}>BRUTO</div>
          </div>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:26, fontWeight:600, color: COLORS.ok, lineHeight:1 }}>{fmtN(Math.round(netoEstimado))}</div>
            <div style={{ fontFamily: FONT.heading, fontSize:10, letterSpacing:'1.5px', textTransform:'uppercase', color: COLORS.ok, marginTop:4 }}>NETO EST. · {(NETO_FACTOR*100).toFixed(0)}%</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:24, borderTop:`0.5px solid ${COLORS.brd}`, paddingTop:12, flexWrap:'wrap' }}>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:18, fontWeight:600, color: COLORS.sec }}>{fmtN(Math.round(mediadiaria))}</div>
            <div style={{ ...lblXs, color: COLORS.mut }}>MEDIA DIARIA BRUTA</div>
          </div>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:18, fontWeight:600, color: COLORS.ok }}>{fmtN(Math.round(mediaDiariaNeta))}</div>
            <div style={{ fontFamily: FONT.heading, fontSize:10, letterSpacing:'1.5px', textTransform:'uppercase', color: COLORS.ok }}>MEDIA DIARIA NETA</div>
          </div>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:18, fontWeight:600, color: COLORS.mut }}>{dias}</div>
            <div style={{ ...lblXs, color: COLORS.mut }}>DÍAS</div>
          </div>
        </div>
      </div>
      {/* Card 2: Pedidos + TM */}
      <div style={CARDS.big}>
        <div style={{ ...lblSm, marginBottom:14 }}>PEDIDOS · TM</div>
        <div style={{ display:'flex', alignItems:'baseline', gap:16, marginBottom:8, flexWrap:'wrap' }}>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:38, fontWeight:600, color: COLORS.lun, lineHeight:1 }}>{fmtN(totals.total_pedidos)}</div>
            <div style={{ ...lblXs, marginTop:4 }}>PEDIDOS</div>
          </div>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:26, fontWeight:600, color: COLORS.warn, lineHeight:1 }}>{fmtEurN(tm)}</div>
            <div style={{ fontFamily: FONT.heading, fontSize:10, letterSpacing:'1.5px', textTransform:'uppercase', color: COLORS.warn, marginTop:4 }}>TM BRUTO</div>
          </div>
          <div>
            <div style={{ fontFamily: FONT.heading, fontSize:26, fontWeight:600, color: COLORS.ok, lineHeight:1 }}>{fmtEurN(tmNeto)}</div>
            <div style={{ fontFamily: FONT.heading, fontSize:10, letterSpacing:'1.5px', textTransform:'uppercase', color: COLORS.ok, marginTop:4 }}>TM NETO</div>
          </div>
        </div>
      </div>
    </div>
...
interface DiarioProps {
  allData: RawDiario[]; cols: typeof ALL_COLS
  weekFilter: {year:number;week:number}|null
  onRefresh: ()=>void; onEdit: (r:RawDiario)=>void
  tipoDia: (fecha:string)=>TipoDia
  totals: AggRow; dias: number; tm: number; tmNeto: number
  netoEstimado: number; mediadiaria: number; mediaDiariaNeta: number
}

function TabDiario({ allData, cols, weekFilter, onEdit, tipoDia, totals, dias, tm, tmNeto, netoEstimado, mediadiaria, mediaDiariaNeta }: DiarioProps) {
  ...
  // Subtotal DÍA — encima de las filas del día
  ...
  const subtotalMap = useMemo(()=>{
    ...
    for(const [fecha,count] of fechaCount) if(count>1) m.set(fecha,aggregate(allData.filter(r=>r.fecha===fecha)))
    return m
  },[allData,fechaCount])
  ...
  const rowsConSub = useMemo(()=>{
    ...
    let lastFecha=''
    for(const r of rows){
      if(r.fecha!==lastFecha){ if((fechaCount.get(r.fecha)??1)>1) result.push({type:'subtotal',fecha:r.fecha,agg:subtotalMap.get(r.fecha)!}); lastFecha=r.fecha }
  ...
  const exportar=()=>{
    ...
    downloadCSV('facturacion_diario.csv',headers,rows.map(r=>[r.fecha,r.servicio,r.uber_pedidos,r.uber_bruto,r.glovo_pedidos,r.glovo_bruto,r.je_pedidos,r.je_bruto,r.web_pedidos,r.web_bruto,r.directa_pedidos,r.directa_bruto,r.total_pedidos,r.total_bruto]))
  }
  ...
  const thBase=(col:SortCol,align:'left'|'right'|'center'='left'): CSSProperties=>({
    fontFamily: FONT.heading, fontSize:10, fontWeight:500, letterSpacing:'2px', textTransform:'uppercase',
    textAlign:align, color: sortCol===col?COLORS.redSL:COLORS.mut,
    padding:'10px 12px', background: COLORS.bg, borderBottom:`0.5px solid ${COLORS.brd}`,
    whiteSpace:'nowrap', cursor:'pointer', userSelect:'none',
  })
  const thCol=(col:SortCol,color:string,bg:string): CSSProperties=>({
    ...thBase(col,'center'), background:bg, color: sortCol===col?color:`${color}99`,
  })
  ...
  if(allData.length===0) return (
    <div style={{ ...CARDS.std, padding:48, textAlign:'center' }}>
      <p style={{ color: COLORS.mut, fontFamily: FONT.body, fontSize:13, margin:0 }}>Sin datos de facturación diaria</p>
    </div>
  )

  return (
    <>
      <KpiCards totals={totals} dias={dias} tm={tm} tmNeto={tmNeto} netoEstimado={netoEstimado} mediadiaria={mediadiaria} mediaDiariaNeta={mediaDiariaNeta} />
      ...
      <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:12 }}>
        ...
        {weekFilter && (
          <span style={{ padding:'4px 10px', background:`${COLORS.redSL}12`, color: COLORS.redSL, borderRadius:8, border:`0.5px solid ${COLORS.redSL}30`, fontFamily: FONT.body, fontSize:12 }}>
            S{weekFilter.week}
          </span>
        )}
      </div>
          ...
          <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0, whiteSpace:'nowrap', minWidth:860 }}>
            <thead>
              <tr>
                ...
                {cols.map(c=>(
                  <th key={c.id} colSpan={2} onClick={()=>handleSort(c.id as SortCol)} style={thCol(c.id as SortCol,c.color,c.bg)}>
                    {c.label}{arr(c.id as SortCol)}
                  </th>
                ))}
              ...
              <tr>
                {cols.map(c=>(
                  <Fragment key={c.id}>
                    <th style={{ padding:'5px 10px', textAlign:'center', background:c.bg, borderBottom:`0.5px solid ${COLORS.brd}`, fontFamily: FONT.heading, fontSize:9, letterSpacing:'1.5px', textTransform:'uppercase', color: COLORS.mut, fontWeight:400 }}>Ped</th>
                    <th style={{ padding:'5px 10px', textAlign:'right',  background:c.bg, borderBottom:`0.5px solid ${COLORS.brd}`, fontFamily: FONT.heading, fontSize:9, letterSpacing:'1.5px', textTransform:'uppercase', color: COLORS.mut, fontWeight:400 }}>Bruto</th>
                  </Fragment>
                ))}
            ...
            <tbody>
              {rowsConSub.map((item,idx)=>{
                if(item.type==='subtotal'){
                  const s=item.agg
                  return (
                    <tr key={`sub-${item.fecha}`} style={{ background: COLORS.bg }}>
                      <td style={{ ...tdBase, color: COLORS.mut, fontSize:11, fontFamily: FONT.heading, letterSpacing:'1px', paddingLeft:16 }}>{fmtFechaCorta(item.fecha)}</td>
                      <td style={tdBase}><ServicioBadge s="TODO" /></td>
                      {cols.map(c=>{
                        const p=(s[c.ped] as number)||0; const b=(s[c.bru] as number)||0
                        return (
                          <Fragment key={c.id}>
                            <td style={{ ...tdBase, textAlign:'center', background:c.bg, color:p>0?c.color:COLORS.mut, fontFamily: FONT.heading, fontSize:12, fontWeight:600 }}>{p>0?fmtN(p):'—'}</td>
                            <td style={{ ...tdBase, textAlign:'right',  background:c.bg, color:b>0?c.color:COLORS.mut, fontFamily: FONT.heading, fontSize:12, fontWeight:600 }}>{b>0?fmtEurN(b):'—'}</td>
                          </Fragment>
                        )
                      })}
                      <td style={{ ...tdBase, textAlign:'center', fontFamily: FONT.heading, fontSize:12, fontWeight:700, color: COLORS.sec }}>{fmtN(s.total_pedidos)}</td>
                      <td style={{ ...tdBase, textAlign:'right',  fontFamily: FONT.heading, fontSize:13, fontWeight:700, color: COLORS.sec }}>{fmtEurN(s.total_bruto)}</td>
                    </tr>
                  )
                }
                const {r}=item
                ...
                const isLast=idx===rowsConSub.length-1
                return (
                  <tr key={r.id} onClick={()=>onEdit(r)} style={{ cursor:'pointer', opacity:esCerrado?0.6:1 }}
                    onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background=`${COLORS.bg}80`}}
                    onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=''}}>
                    <td style={{ ...tdBase, color: COLORS.mut, fontSize:12, borderBottom:isLast?'none':`0.5px solid ${COLORS.brd}`, paddingLeft:16 }}>
                    ...
                    {cols.map(c=>{
                      const p=(r[c.ped] as number)||0; const b=(r[c.bru] as number)||0
                      return (
                        <Fragment key={c.id}>
                          <td style={{ ...tdBase, textAlign:'center', background:c.bg, color:p>0?COLORS.sec:COLORS.mut, borderBottom:isLast?'none':`0.5px solid ${COLORS.brd}` }}>{p>0?fmtN(p):'—'}</td>
                          <td style={{ ...tdBase, textAlign:'right',  background:c.bg, color:b>0?COLORS.sec:COLORS.mut, borderBottom:isLast?'none':`0.5px solid ${COLORS.brd}` }}>{b>0?fmtEurN(b):'—'}</td>
                        </Fragment>
                      )
                    })}
                    <td style={{ ...tdBase, textAlign:'center', fontWeight:500, borderBottom:isLast?'none':`0.5px solid ${COLORS.brd}` }}>{fmtN(r.total_pedidos)}</td>
                    <td style={{ ...tdBase, textAlign:'right',  fontWeight:600, borderBottom:isLast?'none':`0.5px solid ${COLORS.brd}` }}>{fmtEurN(r.total_bruto)}</td>
                  </tr>
                )
              })}
            </tbody>
            <tfoot>
              <tr style={{ background: COLORS.bg }}>
                ...
                {cols.map(c=>(
                  <Fragment key={c.id}>
                    <td style={{ padding:'10px 12px', textAlign:'center', color:c.color, background:c.bg, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtN(rowTotals[c.ped] as number)}</td>
                    <td style={{ padding:'10px 12px', textAlign:'right',  color:c.color, background:c.bg, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtEurN(rowTotals[c.bru] as number)}</td>
                  </Fragment>
                ))}
                <td style={{ padding:'10px 12px', textAlign:'center', color: COLORS.sec, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtN(rowTotals.total_pedidos)}</td>
                <td style={{ padding:'10px 12px', textAlign:'right',  color: COLORS.sec, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtEurN(rowTotals.total_bruto)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
...
// ─── Tab Semanas ───────────────────────────────────────────────
function TabSemanas({ allData, cols, onDrill, totals }: { allData:RawDiario[]; cols:typeof ALL_COLS; onDrill:(y:number,w:number)=>void; totals:AggRow }) {
  const rows = useMemo(()=>buildSemanas(allData).slice(0,12),[allData])
  const exportar=()=>{ const h=['Semana','Periodo','Dias',...cols.map(c=>c.label),'Total Ped','Total Bruto']; downloadCSV('facturacion_semanas.csv',h,rows.map(r=>[`S${r.week}`,r.periodo,r.dias,...cols.map(c=>r[c.bru] as number),r.total_pedidos,r.total_bruto])) }
  ...
  return (
    <>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14, marginBottom:14 }}>
        {[{l:'Semanas',v:String(rows.length)},{l:'Facturación Bruta',v:fmtN(Math.round(totals.total_bruto))},{l:'Pedidos',v:fmtN(totals.total_pedidos)}].map(k=>(
          <div key={k.l} style={CARDS.big}>
            <div style={{ ...lblSm, marginBottom:10 }}>{k.l}</div>
            <div style={{ fontFamily: FONT.heading, fontSize:26, fontWeight:600, color: COLORS.sec }}>{k.v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...CARDS.std, padding:0, overflow:'hidden' }}>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0, whiteSpace:'nowrap' }}>
            <thead>
              <tr>
                <th style={thS}>Sem</th><th style={thS}>Periodo</th><th style={{ ...thS, textAlign:'center' }}>Días</th>
            ...
            <tbody>
              {rows.map((r,idx)=>(
                <tr key={`${r.year}-${r.week}`} onClick={()=>onDrill(r.year,r.week)} style={{ cursor:'pointer' }}
                  onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background=`${COLORS.bg}80`}}
                  onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=''}}>
                  ...
                  {cols.map(c=>(<td key={c.id} style={{ ...tdS, textAlign:'right', background:c.bg, color:(r[c.bru] as number)>0?COLORS.sec:COLORS.mut, borderBottom:idx===rows.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{(r[c.bru] as number)>0?fmtEurN(r[c.bru] as number):'—'}</td>))}
                  <td style={{ ...tdS, textAlign:'right', fontWeight:600, borderBottom:idx===rows.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{fmtEurN(r.total_bruto)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr style={{ background: COLORS.bg }}>
                <td style={{ padding:'10px 12px', color: COLORS.mut, fontFamily: FONT.heading, fontSize:10, letterSpacing:'2px', textTransform:'uppercase', borderTop:`0.5px solid ${COLORS.brd}` }} colSpan={3}>Total</td>
                {cols.map(c=>(<td key={c.id} style={{ padding:'10px 12px', textAlign:'right', color:c.color, background:c.bg, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtEurN(totals[c.bru] as number)}</td>))}
                <td style={{ padding:'10px 12px', textAlign:'right', color: COLORS.sec, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtEurN(totals.total_bruto)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
...
// ─── Tab Meses ─────────────────────────────────────────────────
function TabMeses({ allData, cols, totals }: { allData:RawDiario[]; cols:typeof ALL_COLS; totals:AggRow }) {
  ...
  const yearTotal=useMemo(()=>{const a=aggregate(allData.filter(r=>r.fecha.startsWith(String(selYear))));const dias=new Set(allData.filter(r=>r.fecha.startsWith(String(selYear))).map(r=>r.fecha)).size;return{...a,dias}},[allData,selYear])

  const exportar=()=>{ const h=['Mes','Dias',...cols.map(c=>c.label),'Total Ped','Total Bruto','Media Diaria','vs Anterior']; downloadCSV(`facturacion_meses_${selYear}.csv`,h,rows.map(r=>{const vs=r.vs_anterior!==null?r.vs_anterior.toFixed(1)+'%':'';return[MES_NOMBRE[r.mes],r.dias,...cols.map(c=>r[c.bru] as number),r.total_pedidos,r.total_bruto,r.media_diaria.toFixed(2),vs]})) }
  const thS: CSSProperties={ fontFamily: FONT.heading, fontSize:10, fontWeight:500, letterSpacing:'2px', textTransform:'uppercase', color: COLORS.mut, padding:'10px 12px', background: COLORS.bg, borderBottom:`0.5px solid ${COLORS.brd}`, whiteSpace:'nowrap' }
  const tdS: CSSProperties={ padding:'9px 12px', fontSize:13, fontFamily: FONT.body, color: COLORS.sec, borderBottom:`0.5px solid ${COLORS.brd}`, whiteSpace:'nowrap' }
  ...
  return (
    <>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14, marginBottom:14 }}>
        {[{l:'Facturación Bruta',v:fmtN(Math.round(yearTotal.total_bruto))},{l:'Pedidos',v:fmtN(yearTotal.total_pedidos)},{l:'Media Diaria',v:yearTotal.dias>0?fmtN(Math.round(yearTotal.total_bruto/yearTotal.dias)):'—'}].map(k=>(
          <div key={k.l} style={CARDS.big}>
            <div style={{ ...lblSm, marginBottom:10 }}>{k.l}</div>
            <div style={{ fontFamily: FONT.heading, fontSize:26, fontWeight:600, color: COLORS.sec }}>{k.v}</div>
          </div>
      ...
      <div style={{ ...CARDS.std, padding:0, overflow:'hidden' }}>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0, whiteSpace:'nowrap' }}>
            <thead>
              <tr>
                <th style={thS}>Mes</th><th style={{ ...thS, textAlign:'center' }}>Días</th>
                {cols.map(c=><th key={c.id} style={{ ...thS, background:c.bg, color:`${c.color}99`, textAlign:'right' }}>{c.label}</th>)}
            ...
            <tbody>
              {rows.map((r,idx)=>(
                <tr key={r.mes} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background=`${COLORS.bg}80`}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=''}}>
                  ...
                  <td style={{ ...tdS, textAlign:'center', color: COLORS.mut, borderBottom:idx===rows.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{r.dias}</td>
                  {cols.map(c=>(<td key={c.id} style={{ ...tdS, textAlign:'right', background:c.bg, color:(r[c.bru] as number)>0?COLORS.sec:COLORS.mut, borderBottom:idx===rows.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{(r[c.bru] as number)>0?fmtEurN(r[c.bru] as number):'—'}</td>))}
                  <td style={{ ...tdS, textAlign:'right', color: COLORS.mut, borderBottom:idx===rows.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{r.dias>0?fmtEurN(r.media_diaria):'—'}</td>
            ...
            <tfoot>
              <tr style={{ background: COLORS.bg }}>
                <td style={{ padding:'10px 12px', color: COLORS.mut, fontFamily: FONT.heading, fontSize:10, letterSpacing:'2px', textTransform:'uppercase', borderTop:`0.5px solid ${COLORS.brd}` }} colSpan={2}>{selYear} Total</td>
                {cols.map(c=>(<td key={c.id} style={{ padding:'10px 12px', textAlign:'right', color:c.color, background:c.bg, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{fmtEurN(yearTotal[c.bru] as number)}</td>))}
                <td style={{ padding:'10px 12px', textAlign:'right', color: COLORS.sec, fontFamily: FONT.heading, fontSize:11, fontWeight:600, borderTop:`0.5px solid ${COLORS.brd}` }}>{yearTotal.dias>0?fmtEurN(yearTotal.total_bruto/yearTotal.dias):'—'}</td>
                <td style={{ padding:'10px 12px', borderTop:`0.5px solid ${COLORS.brd}` }} />
              </tr>
            </tfoot>
          </table>
        </div>
...
// ─── Tab Anual ─────────────────────────────────────────────────
function TabAnual({ allData }: { allData:RawDiario[] }) {
  const years = useMemo(()=>{
    const m=new Map<number,{bruto:number;pedidos:number}>()
    for(const r of allData){ const y=parseInt(r.fecha.slice(0,4)); if(!m.has(y))m.set(y,{bruto:0,pedidos:0}); const c=m.get(y)!; c.bruto+=r.total_bruto||0; c.pedidos+=r.total_pedidos||0 }
    return [...m.entries()].sort((a,b)=>b[0]-a[0]).map(([anio,v])=>({ anio, bruto:v.bruto, pedidos:v.pedidos, mediaMensual:v.bruto/12, mediaTicket:v.pedidos>0?v.bruto/v.pedidos:0 }))
  },[allData])
  ...
  return (
    <div>
      {years.length>0&&(()=>{
        ...
        const dTicket=prev&&prev.mediaTicket>0?((cur.mediaTicket-prev.mediaTicket)/prev.mediaTicket)*100:null
        return (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:20 }}>
            {[{l:`Facturación ${cur.anio}`,v:fmtN(Math.round(cur.bruto)),d:delta},{l:'Media mensual',v:fmtN(Math.round(cur.mediaMensual)),d:null},{l:'Pedidos totales',v:fmtN(cur.pedidos),d:null},{l:'Ticket medio',v:fmtEurN(cur.mediaTicket),d:dTicket}].map(k=>(
              <div key={k.l} style={CARDS.big}>
                <div style={{ ...lblSm, marginBottom:10 }}>{k.l}</div>
                <div style={{ fontFamily: FONT.heading, fontSize:26, fontWeight:600, color: COLORS.sec }}>{k.v}</div>
                {k.d!=null&&(<div style={{ fontFamily: FONT.body, fontSize:12, color:k.d>=0?COLORS.ok:COLORS.err, marginTop:4 }}>{k.d>=0?'▲':'▼'} {Math.abs(k.d).toFixed(1)}% vs {cur.anio-1}</div>)}
              </div>
            ))}
          </div>
        )
      ...
      <div style={{ ...CARDS.std, padding:0, overflow:'hidden' }}>
        <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0 }}>
          ...
          <tbody>
            {years.map((y,idx)=>{
              const prev=years[idx+1]; const delta=prev?((y.bruto-prev.bruto)/prev.bruto)*100:null
              const barW=`${Math.round((y.bruto/maxBruto)*100)}%`
              return (
                <tr key={y.anio} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background=`${COLORS.bg}80`}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=''}}>
                  ...
                  <td style={{ ...tdS, textAlign:'right', borderBottom:idx===years.length-1?'none':`0.5px solid ${COLORS.brd}` }}>
                    <div style={{ fontFamily: FONT.heading, fontSize:14, fontWeight:600, marginBottom:4 }}>{fmtEurN(y.bruto)}</div>
                    <div style={{ height:4, background: COLORS.brd, borderRadius:2, overflow:'hidden' }}><div style={{ height:4, width:barW, background: COLORS.redSL, borderRadius:2 }} /></div>
                  </td>
                  ...
                  <td style={{ ...tdS, textAlign:'right', color: COLORS.mut, borderBottom:idx===years.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{fmtEurN(y.mediaMensual)}</td>
                  <td style={{ ...tdS, textAlign:'right', color: COLORS.mut, borderBottom:idx===years.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{fmtN(y.pedidos)}</td>
                  <td style={{ ...tdS, textAlign:'right', color: COLORS.mut, borderBottom:idx===years.length-1?'none':`0.5px solid ${COLORS.brd}` }}>{fmtEurN(y.mediaTicket)}</td>
                </tr>
              )
            })}
          </tbody>
...
const FORM_COLS: { label:string; ped:keyof FormFields; bru:keyof FormFields }[] = [
  {label:'Uber Eats',ped:'uber_pedidos',bru:'uber_bruto'},{label:'Glovo',ped:'glovo_pedidos',bru:'glovo_bruto'},
  {label:'Web',ped:'web_pedidos',bru:'web_bruto'},{label:'Venta Directa',ped:'directa_ped',bru:'directa_bru'},
]
const CANAL_COLORS_MODAL: Record<string,{bg:string;border:string;label:string}> = {
  'Uber Eats':     {bg:'#06C16712',border:'#06C167',label:'#06C167'},
  'Glovo':         {bg:'#e8f44218',border:'#8a7800',label:'#8a7800'},
...
function DayModal({ allData, existing, onClose, onSaved }: { allData:RawDiario[]; existing?:RawDiario; onClose:()=>void; onSaved:()=>void }) {
  ...
  const [fields,setFields]=useState<FormFields>(()=>{
    ...
    return {uber_pedidos:String(existing.uber_pedidos||''),uber_bruto:String(existing.uber_bruto||''),glovo_pedidos:String(existing.glovo_pedidos||''),glovo_bruto:String(existing.glovo_bruto||''),je_ped:String(existing.je_pedidos||''),je_bru:String(existing.je_bruto||''),web_pedidos:String(existing.web_pedidos||''),web_bruto:String(existing.web_bruto||''),directa_ped:String(existing.directa_pedidos||0),directa_bru:String(existing.directa_bruto||0)}
  })
  ...
  const handleSubmit=async(e:FormEvent)=>{
    ...
    if(servicio==='CENAS_ALM'){
      ...
      if(tp===0&&tb===0){setFormError('El total es igual o menor al ALM.');return}
      ...
      const{error}=await supabase.from('facturacion_diario').insert(p); setSaving(false)
      if(error){setFormError(error.message);return}; onSaved(); return
    }
    ...
    setSaving(true)
    const{error}=isEdit?await supabase.from('facturacion_diario').update(p).eq('id',existing!.id):await supabase.from('facturacion_diario').insert(p)
    setSaving(false); if(error){setFormError(error.message);return}; onSaved()
  }

  const handleDelete=async()=>{
    if(!confirm('¿Eliminar este día?'))return
    const{error}=await supabase.from('facturacion_diario').delete().eq('id',existing!.id)
    if(error){setFormError(error.message);return}; onSaved()
  }
        ...
        <form onSubmit={handleSubmit} style={{ padding:20, display:'flex', flexDirection:'column', gap:16 }}>
          <div style={{ display:'flex', gap:12, alignItems:'flex-end' }}>
            <div style={{ flex:'0 0 43%' }}>
              <label style={{ display:'block', fontSize:11, color: COLORS.mut, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.5px', fontFamily: FONT.heading }}>Fecha</label>
              <input type="date" value={fecha} onChange={e=>setFecha(e.target.value)} style={inp} />
            </div>
            <div style={{ flex:1 }}>
              <label style={{ display:'block', fontSize:11, color: COLORS.mut, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.5px', fontFamily: FONT.heading }}>Servicio</label>
              <div style={{ display:'flex', gap:4 }}>
                {[{key:'TODO',label:'TODOS'},{key:'ALM',label:'ALM'},{key:'CENAS',label:'CENAS'},{key:'CENAS_ALM',label:'CENAS/ALM'}].map(s=>{
                  const isActive=servicio===s.key; const isCenasAlm=s.key==='CENAS_ALM'
                  return (<button key={s.key} type="button" onClick={()=>setServicio(s.key)} style={{ flex:1, padding:'8px 4px', borderRadius:8, fontSize:10, fontWeight:600, border:isActive?'none':`0.5px solid ${COLORS.brd}`, background:isActive?(isCenasAlm?'#7c3aed':COLORS.redSL):'#fff', color:isActive?'#fff': COLORS.mut, cursor:'pointer', fontFamily: FONT.heading, letterSpacing:'0.5px', whiteSpace:'nowrap' }}>{s.label}</button>)
                })}
              </div>
            </div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {FORM_COLS.slice(0,2).map(c=>{const cc=CANAL_COLORS_MODAL[c.label];return(<div key={c.label} style={{ background:cc?.bg??'#f5f5f5', border:`1px solid ${cc?.border??'#ccc'}`, borderRadius:10, padding:12 }}><p style={{ fontSize:11, fontWeight:600, marginBottom:10, color:cc?.label??'#666', fontFamily: FONT.heading, letterSpacing:1, textTransform:'uppercase' }}>{c.label}</p><div style={{ display:'flex', flexDirection:'column', gap:10 }}><div><label style={{ display:'block', fontSize:10, color: COLORS.mut, marginBottom:4 }}>Pedidos</label><input type="number" min="0" placeholder="0" value={fields[c.ped]} onChange={e=>set(c.ped,e.target.value)} style={inp} /></div><div><label style={{ display:'block', fontSize:10, color: COLORS.mut, marginBottom:4 }}>Bruto (EUR)</label><input type="number" min="0" step="0.01" placeholder="0.00" value={fields[c.bru]} onChange={e=>set(c.bru,e.target.value)} style={inp} /></div></div></div>)})}
          </div>
          <div style={{ background:'#f5a62312', border:'1px solid #f5a623', borderRadius:10, padding:14 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
              ...
              {jeItems.length>0&&<span style={{ fontFamily: FONT.body, fontSize:12, color: COLORS.mut }}>{jeItems.length} pedido{jeItems.length!==1?'s':''} · {jeItems.reduce((a,b)=>a+b,0).toFixed(2)} €</span>}
            </div>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center' }}>
              <input type="number" step="0.01" min="0" placeholder="Importe (€)" value={jeInput} onChange={e=>setJeInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();const v=parseFloat(jeInput);if(v>0){setJeItems(p=>[...p,v]);setJeInput('')}}}} style={{ ...inp, flex:1, width:'auto', padding:'6px 10px' }} />
              <button type="button" onClick={()=>{const v=parseFloat(jeInput);if(v>0){setJeItems(p=>[...p,v]);setJeInput('')}}} style={{ padding:'6px 14px', borderRadius:8, background:'#f5a623', color:'#fff', border:'none', cursor:'pointer', fontFamily: FONT.heading, fontSize:14, fontWeight:600, flexShrink:0 }}>+</button>
            </div>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {FORM_COLS.slice(2).map(c=>{const cc=CANAL_COLORS_MODAL[c.label];return(<div key={c.label} style={{ background:cc?.bg??'#f5f5f5', border:`1px solid ${cc?.border??'#ccc'}`, borderRadius:10, padding:12 }}><p style={{ fontSize:11, fontWeight:600, marginBottom:10, color:cc?.label??'#666', fontFamily: FONT.heading, letterSpacing:1, textTransform:'uppercase' }}>{c.label}</p><div style={{ display:'flex', flexDirection:'column', gap:10 }}><div><label style={{ display:'block', fontSize:10, color: COLORS.mut, marginBottom:4 }}>Pedidos</label><input type="number" min="0" placeholder="0" value={fields[c.ped]} onChange={e=>set(c.ped,e.target.value)} style={inp} /></div><div><label style={{ display:'block', fontSize:10, color: COLORS.mut, marginBottom:4 }}>Bruto (EUR)</label><input type="number" min="0" step="0.01" placeholder="0.00" value={fields[c.bru]} onChange={e=>set(c.bru,e.target.value)} style={inp} /></div></div></div>)})}
          </div>
...
function ServicioBadge({ s }: { s:string }) {
  const color = s==='ALM'?COLORS.warn:s==='CENAS'?'#7c3aed':COLORS.mut
