...
export default function Facturacion() {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-white mb-4">Facturacion</h2>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <select
          ...
          className="bg-base border border-border rounded-lg px-3 py-2 text-sm text-white"
        >
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <button onClick={exportar} className="px-3 py-2 text-xs text-neutral-400 border border-border rounded-lg hover:text-white transition">
          Exportar CSV
        </button>
            ...
            <thead>
              {showBreakdown ? (
                <>
                  ...
                  <tr className="border-b border-border text-neutral-600 text-[10px] uppercase tracking-wider">
                    {COLS.map(c => (
            ...
            <tbody className="divide-y divide-border">
              {rows.map(r => (
                <tr key={r.id} onClick={() => onEdit(r)}
                  ...
                  <td className="px-3 py-2 text-white sticky left-0 bg-card">{r.fecha}</td>
                  ...
                  {showBreakdown ? (
                    <>
                      {COLS.map(c => {
                        ...
                        return (
                          <Fragment key={c.label}>
                            <td className="px-2 py-2 text-right text-neutral-400 tabular-nums border-l border-border">
                              {p > 0 ? p : <Dash />}
                            </td>
                      ...
                      <td className="px-2 py-2 text-right text-white font-medium tabular-nums border-l border-border">{num(r.total_pedidos)}</td>
                      <td className="px-2 py-2 text-right text-accent font-semibold tabular-nums">{eur(r.total_bruto)}</td>
                    </>
                  ) : (
                    <>
                      <td className="px-3 py-2 text-right text-white tabular-nums">{num(getPed(r, canal))}</td>
              ...
              <tr className="border-t-2 border-accent/30 bg-accent/5 font-semibold">
                ...
                {showBreakdown ? (
                  <>
                    ...
                    <td className="px-2 py-2.5 text-right text-white tabular-nums border-l border-border">{num(totals.total_pedidos)}</td>
                    <td className="px-2 py-2.5 text-right text-accent tabular-nums">{eur(totals.total_bruto)}</td>
                  </>
                ) : (
                  <>
                    <td className="px-3 py-2.5 text-right text-white tabular-nums">{num(getPed(totals, canal))}</td>
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <MiniKpi label="Bruto" valor={eur(getBru(totals, canal))} />
        <MiniKpi label="Pedidos" valor={num(getPed(totals, canal))} />
        <button onClick={exportar} className="ml-auto px-3 py-2 text-xs text-neutral-400 border border-border rounded-lg hover:text-white transition">
          Exportar CSV
        </button>
      </div>
          ...
          <table className="w-full text-sm whitespace-nowrap">
              ...
              {rows.map(r => {
                ...
                return (
                  <tr key={`${r.year}-${r.week}`} onClick={() => onDrill(r.year, r.week)}
                    className="hover:bg-white/[0.03] transition-colors cursor-pointer">
                    <td className="px-4 py-2.5 text-white font-medium">S{r.week}</td>
                    <td className="px-4 py-2.5 text-neutral-400">{r.periodo}</td>
                    <td className="px-3 py-2.5 text-right text-neutral-500 tabular-nums">{r.dias}</td>
                    <td className="px-4 py-2.5 text-right text-white tabular-nums">{num(ped)}</td>
                    <td className="px-4 py-2.5 text-right text-accent font-medium tabular-nums">{eur(bru)}</td>
                    {showBreakdown && COLS.map(c => (
                      <td key={c.label} className="px-3 py-2.5 text-right text-neutral-400 tabular-nums border-l border-border">
            ...
            <tfoot>
              <tr className="border-t-2 border-accent/30 bg-accent/5 font-semibold">
                ...
                <td className="px-4 py-2.5 text-right text-white tabular-nums">{num(getPed(totals, canal))}</td>
      ...
      <p className="text-[10px] text-neutral-600 mt-2">Haz clic en una semana para ver el detalle diario</p>
    </>
  )
}
          ...
          <table className="w-full text-sm whitespace-nowrap">
              ...
              {rows.map(r => {
                ...
                return (
                  <tr key={r.mes} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-2.5 text-white font-medium">{MES_NOMBRE[r.mes]}</td>
                    <td className="px-3 py-2.5 text-right text-neutral-500 tabular-nums">{r.dias}</td>
                    <td className="px-4 py-2.5 text-right text-white tabular-nums">{num(ped)}</td>
                    <td className="px-4 py-2.5 text-right text-accent font-medium tabular-nums">{eur(bru)}</td>
                    {showBreakdown && COLS.map(c => (
                    ...
                    <td className="px-4 py-2.5 text-right text-neutral-400 tabular-nums border-l border-border">
            ...
            <tfoot>
              <tr className="border-t-2 border-accent/30 bg-accent/5 font-semibold">
                ...
                <td className="px-3 py-2.5 text-right text-neutral-400 tabular-nums">{yearTotal.dias}</td>
                <td className="px-4 py-2.5 text-right text-white tabular-nums">{num(getPed(yearTotal, canal))}</td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h3 className="text-white font-semibold">{isEdit ? 'Editar dia' : 'Anadir dia'}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={fecha} onChange={e => setFecha(e.target.value)}
                className="w-full bg-base border border-border rounded-lg px-3 py-2.5 text-sm text-white" />
            </div>
            <div>
              <label className="block text-xs text-neutral-500 mb-1">Servicio</label>
              <div className="flex gap-1">
                {SERVICIOS.map(s => (
                  <button key={s} type="button" onClick={() => setServicio(s)}
                    className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                      servicio === s ? 'bg-accent text-black' : 'bg-base border border-border text-neutral-400 hover:text-white'
                    }`}>{s}</button>
                ))}
              </div>
            </div>
          ...
          {FORM_COLS.map(c => (
            <div key={c.label} className="bg-base/50 border border-border rounded-lg p-3">
              <p className="text-xs text-neutral-400 font-medium mb-2">{c.label}</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] text-neutral-600 mb-0.5">Pedidos</label>
                ...
                <div>
                  <label className="block text-[10px] text-neutral-600 mb-0.5">Bruto (EUR)</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 rounded-lg text-sm text-neutral-400 border border-border hover:text-white transition">Cancelar</button>
