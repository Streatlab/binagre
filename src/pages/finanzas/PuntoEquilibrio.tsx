/**
 * Punto de Equilibrio — refactor v5 — 19 may 2026
 *
 * Datos REALES verificados en Supabase:
 *   - Bruto + pedidos: facturacion_diario (uber_bruto, glovo_bruto, je_bruto, web_bruto, directa_bruto, total_pedidos, *_pedidos)
 *   - Neto por canal: resumenes_plataforma_marca_mensual (neto_real_cobrado, o bruto-com-fees-cargos-21%IVA)
 *   - Gastos SIN IVA: tabla gastos campo base_imponible (importes vienen NEGATIVOS, usar Math.abs)
 *   - Grupos reales BD: 'Producto' (variable), 'Equipo'+'Alquiler'+'Controlables' (fijos)
 *
 * Estilo: cards Panel Global (cardBig + lbl + lblXs + OSWALD), bruto NEGRO + neto VERDE + margen %
 * Formato: SIEMPRE 2 decimales + separador miles es-ES, sin €
 */
...
// Grupos reales BD (capitalizados)
...
export default function PuntoEquilibrio() {
  ...
  const periodo = useMemo(() => ({
    desde: periodoDesde, hasta: periodoHasta, key: 'pe', label: periodoLabel,
  }), [periodoDesde, periodoHasta, periodoLabel])
  const anio = periodo.desde.getFullYear()

  // useRunning con IVA SIN forzado (base_imponible)
  const { loading, error, gastos, facturacion } = useRunning(
    periodo, anio, null, null, 'sin',
  )
  const { diasOperativosEnRango } = useCalendario()

  // Bruto y pedidos por canal desde facturacion_diario
  const brutoPorCanal = useMemo(() => {
    const m: Record<CanalId, number> = { uber: 0, glovo: 0, just_eat: 0, web: 0, directa: 0 }
    const ped: Record<CanalId, number> = { uber: 0, glovo: 0, just_eat: 0, web: 0, directa: 0 }
  ...
  // Cargar resumenes para neto real
  const meses = useMemo(() => {
  ...
  const datosPorCanal = useMemo<Record<CanalId, CanalDatos>>(() => {
    const out = {} as Record<CanalId, CanalDatos>
    const ids: CanalId[] = ['uber','glovo','just_eat','web','directa']
    for (const c of ids) {
      const filas = resumenes.filter(r => r.plataforma === c)
      const brutoFD = brutoPorCanal.bruto[c]
      const pedFD = brutoPorCanal.pedidos[c]
      let neto = 0
      let brutoCalc = 0
      if (filas.length > 0) {
        const tieneReal = filas.some(f => f.neto_real_cobrado != null)
      ...
      out[c] = { bruto: brutoFinal, neto, margenPct, pedidos: pedFD }
    }
    return out
  ...
  // Gastos por GRUPO real (no la enum vieja). Math.abs porque vienen negativos.
  const gastosPorGrupo = useMemo(() => {
    const m: Record<string, number> = {}
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    for (const gr of gastos as any[]) {
      const g = gr as { categoria?: string; importe?: number }
      const grupo = String(g.categoria ?? '')
      if (!grupo) continue
      const importe = Math.abs(Number(g.importe ?? 0))
      m[grupo] = (m[grupo] ?? 0) + importe
    }
    return m
  }, [gastos])

  const totalFijos = useMemo(
    () => GRUPOS_FIJOS.reduce((a, g) => a + (gastosPorGrupo[g] ?? 0), 0),
    [gastosPorGrupo]
  )

  const totalVariablesGasto = useMemo(
    () => GRUPOS_VARIABLES.reduce((a, g) => a + (gastosPorGrupo[g] ?? 0), 0),
    [gastosPorGrupo]
  )
  ...
  // Día/Mes que se cubre el PE al ritmo actual (avanza por días operativos)
  const diaCubreInfo = useMemo(() => {
    if (!peMensual || brutoMedioDiario <= 0) {
      return { fecha: null as Date | null, diasNecesarios: null as number | null, mesesDelta: 0 }
    }
    const diasNecesarios = Math.ceil(peMensual / brutoMedioDiario)
    const inicio = new Date(periodo.desde.getFullYear(), periodo.desde.getMonth(), 1)
    const cur = new Date(inicio)
    let contados = 0
    let safety = 0
    while (contados < diasNecesarios && safety < 730) {
      if (diasOperativosEnRango(cur, cur) === 1) contados++
      if (contados >= diasNecesarios) break
      cur.setDate(cur.getDate() + 1)
      safety++
    }
    const mesesDelta = (cur.getFullYear() - inicio.getFullYear()) * 12 + (cur.getMonth() - inicio.getMonth())
    return { fecha: cur, diasNecesarios, mesesDelta }
  ...
  return (
    <div style={{ background: '#f5f3ef', padding: '24px 28px', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{
            color: ROJO, fontFamily: OSWALD, fontSize: 22, fontWeight: 600,
      ...
      {!loading && !error && tab === 'resumen' && (
        <TabResumen
          totalBruto={totalBruto} totalNeto={totalNeto} totalPedidos={totalPedidos}
          totalFijos={totalFijos} totalComisiones={totalComisiones} totalVariables={totalVariables}
          margenContribPct={margenContribPct} margenNetoPct={margenNetoPct}
          peMensual={peMensual} diaCubreInfo={diaCubreInfo}
          datosPorCanal={datosPorCanal} gastosPorGrupo={gastosPorGrupo}
          diasOperativos={diasOperativos} brutoMedioDiario={brutoMedioDiario}
          colorEstado={colorEstado} beneficio={beneficio} ebitdaPct={ebitdaPct}
          ticketMedioBruto={ticketMedioBruto} ticketMedioNeto={ticketMedioNeto}
        />
      )}

      {!loading && !error && tab === 'simulador' && (
        <TabSimulador
          totalBruto={totalBruto} totalFijos={totalFijos}
          margenContribPct={margenContribPct} peMensual={peMensual}
          totalPedidos={totalPedidos} brutoMedioDiario={brutoMedioDiario}
        />
      )}
    </div>
  )
...
interface TabResumenProps {
  ...
  diaCubreInfo: { fecha: Date | null; diasNecesarios: number | null; mesesDelta: number }
  datosPorCanal: Record<CanalId, CanalDatos>
...
function TabResumen(p: TabResumenProps) {
  const filasFijos = GRUPOS_FIJOS
    .map(g => ({ label: g, valor: p.gastosPorGrupo[g] ?? 0, color: COLOR_GRUPO[g] }))
    .filter(f => f.valor > 0)
    .sort((a, b) => b.valor - a.valor)

  const filasVariables = [
    ...GRUPOS_VARIABLES.map(g => ({ label: g, valor: p.gastosPorGrupo[g] ?? 0, color: COLOR_GRUPO[g] })),
    { label: 'Comisiones plataformas', valor: p.totalComisiones, color: '#F26B1F' },
  ].filter(f => f.valor > 0).sort((a, b) => b.valor - a.valor)

  return (
    <div style={{ marginTop: 16 }}>
      {/* Row 1: Facturación grande + PE + Resultado */}
      <div style={{
        display: 'grid',
        ...
        <CardFacturacion
          totalBruto={p.totalBruto} totalNeto={p.totalNeto}
          margenNetoPct={p.margenNetoPct} totalPedidos={p.totalPedidos}
          ticketMedioBruto={p.ticketMedioBruto} ticketMedioNeto={p.ticketMedioNeto}
          datosPorCanal={p.datosPorCanal}
        />

        <CardPE
          peMensual={p.peMensual} totalBruto={p.totalBruto}
          diaCubreInfo={p.diaCubreInfo} colorEstado={p.colorEstado}
          brutoMedioDiario={p.brutoMedioDiario} diasOperativos={p.diasOperativos}
          ticketMedioBruto={p.ticketMedioBruto} margenContribPct={p.margenContribPct}
        />

        <CardResultado
          beneficio={p.beneficio} ebitdaPct={p.ebitdaPct}
          totalBruto={p.totalBruto} totalNeto={p.totalNeto}
          totalFijos={p.totalFijos} totalVariables={p.totalVariables}
          margenContribPct={p.margenContribPct}
        />
      </div>

      {/* Row 2: Costes fijos + variables */}
      <div style={{
        display: 'grid',
        gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))',
        gap: 14,
...
/* ─────────────────────────────────────────────────────────── */
/* CARD FACTURACIÓN — estilo Panel Global                      */
/* ─────────────────────────────────────────────────────────── */

function CardFacturacion({
  totalBruto, totalNeto, margenNetoPct, totalPedidos,
  ticketMedioBruto, ticketMedioNeto, datosPorCanal,
}: {
  totalBruto: number
  totalNeto: number
  margenNetoPct: number
  ...
  return (
    <div style={cardBig}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 18, marginTop: 8, flexWrap: 'wrap' }}>
        ...
        <div>
          <div style={{ fontFamily: OSWALD, fontSize: 38, fontWeight: 600, color: VERDE }}>
      ...
      <div style={{ fontFamily: LEXEND, fontSize: 12, color: '#3a4050', marginTop: 10, marginBottom: 16 }}>
        <strong style={{ fontFamily: OSWALD, fontSize: 14 }}>{fmtEur(totalPedidos, { showEuro: false, decimals: 0 })}</strong> pedidos
        {' · '}TM <strong style={{ fontFamily: OSWALD }}>{fmtEur(ticketMedioBruto, { showEuro: false, decimals: 2 })}</strong> bruto
        {' / '}<strong style={{ fontFamily: OSWALD, color: VERDE }}>{fmtEur(ticketMedioNeto, { showEuro: false, decimals: 2 })}</strong> neto
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {bigs.map(c => {
          const d = datosPorCanal[c.id]
          const pctMix = totalBruto > 0 ? (d.bruto / totalBruto) * 100 : 0
          return (
            <CardCanalBig
              key={c.id}
              label={c.label} bg={c.bg} border={c.border} colorLabel={c.colorLabel}
              bruto={d.bruto} neto={d.neto} margenPct={d.margenPct} pctMix={pctMix}
            />
          )
        })}
...
function CardCanalBig({ label, bg, border, colorLabel, bruto, neto, margenPct, pctMix }: {
  label: string; bg: string; border: string; colorLabel: string
  bruto: number; neto: number; margenPct: number; pctMix: number
}) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ ...lblXs, color: colorLabel }}>
          {label} · {fmtPct(pctMix, 2)}
        </div>
      ...
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontFamily: OSWALD, fontSize: 24, fontWeight: 600, color: VERDE }}>
          {fmtEur(neto, { showEuro: false, decimals: 2 })}
        </div>
        <div style={{ fontSize: 12, color: VERDE, fontFamily: LEXEND }}>
...
function CardCanalMini({ label, bg, border, colorLabel, bruto, neto, margenPct, pctMix }: {
  label: string; bg: string; border: string; colorLabel: string
  bruto: number; neto: number; margenPct: number; pctMix: number
}) {
  return (
    <div style={{
      background: bg, border: `0.5px solid ${border}`, borderRadius: 14, padding: '10px 12px',
    }}>
      <div style={{ ...lblXs, color: colorLabel }}>
        {label} · {fmtPct(pctMix, 2)}
      </div>
      ...
      <div style={{ fontSize: 10, color: COLOR.textMut, fontFamily: LEXEND }}>
        {fmtEur(neto, { showEuro: false, decimals: 2 })} neto · {fmtPct(margenPct, 2)}
      </div>
    </div>
  )
}
...
function CardPE({
  peMensual, totalBruto, diaCubreInfo, colorEstado,
  brutoMedioDiario, diasOperativos, ticketMedioBruto, margenContribPct,
}: {
  peMensual: number | null
  totalBruto: number
  diaCubreInfo: { fecha: Date | null; diasNecesarios: number | null; mesesDelta: number }
  ...
  const mesesDelta = diaCubreInfo.mesesDelta

  // Círculo: si mesesDelta=0 → día + mes corto; si >0 → +NM + mes
  let circuloLinea1 = '—'
  let circuloLinea2 = ''
  ...
  if (fechaPE && peMensual) {
    if (mesesDelta === 0) {
      ...
      circuloLinea2 = MESES_CORTO[fechaPE.getMonth()]
      // si pasa 1+ meses = ya es barbaridad
      if (mesesDelta >= 1) circuloColor = ERR
      else if (mesesDelta === 0 && pctCubierto < 100) circuloColor = AMBAR
    }
  } else {
    circuloColor = ERR
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 18, marginTop: 8, flexWrap: 'wrap' }}>
        ...
        <div>
          <div style={{ fontFamily: OSWALD, fontSize: 22, fontWeight: 600, color: VERDE }}>
        ...
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <div>
            <div style={{ fontFamily: OSWALD, fontSize: 18, fontWeight: 600, color: '#111111' }}>
              {diasNecesarios != null ? fmtEur(diasNecesarios, { showEuro: false, decimals: 0 }) : '—'}
            </div>
            <div style={lblXs}>DÍAS PARA CUBRIR</div>
          </div>
          <div>
            <div style={{ fontFamily: OSWALD, fontSize: 18, fontWeight: 600, color: '#111111' }}>
              {pedidosNecesarios != null ? fmtEur(pedidosNecesarios, { showEuro: false, decimals: 0 }) : '—'}
            </div>
            <div style={lblXs}>PEDIDOS NECES.</div>
          </div>
          <div>
            <div style={{ fontFamily: OSWALD, fontSize: 14, fontWeight: 600, color: '#111111' }}>
              {fmtEur(brutoMedioDiario, { showEuro: false, decimals: 2 })}
            </div>
            <div style={lblXs}>€/DÍA REAL</div>
          </div>
          <div>
            <div style={{ fontFamily: OSWALD, fontSize: 14, fontWeight: 600, color: brutoDiaObjetivo && brutoMedioDiario >= brutoDiaObjetivo ? VERDE : ERR }}>
              {brutoDiaObjetivo != null ? fmtEur(brutoDiaObjetivo, { showEuro: false, decimals: 2 }) : '—'}
            </div>
            <div style={lblXs}>€/DÍA OBJETIVO</div>
          </div>
        </div>
      </div>
      ...
      {peMensual != null && (
        <>
          <div style={{
            ...
            <div style={{
              width: `${pctCubierto}%`, height: '100%', background: colorEstado,
              transition: 'width 0.5s ease',
            }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: LEXEND, fontSize: 11, color: '#7a8090', marginTop: 6 }}>
            <span>Cubierto</span>
            <strong style={{ color: colorEstado, fontFamily: OSWALD, fontSize: 12 }}>{fmtPct(pctCubierto, 2)}</strong>
          </div>
        </>
      )}
    </div>
...
function CardResultado({ beneficio, ebitdaPct, totalBruto, totalNeto, totalFijos, totalVariables, margenContribPct }: {
  beneficio: number
  ebitdaPct: number
  totalBruto: number
  totalNeto: number
  ...
  const filas = [
    { label: 'Bruto',            valor: totalBruto,      color: '#111111', signed: false },
    { label: 'Neto estimado',    valor: totalNeto,       color: VERDE,     signed: false },
    { label: 'Costes variables', valor: -totalVariables, color: '#3a4050', signed: true  },
    { label: 'Costes fijos',     valor: -totalFijos,     color: '#3a4050', signed: true  },
  ]
  ...
  return (
    <div style={cardBig}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 18, marginTop: 8, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontFamily: OSWALD, fontSize: 38, fontWeight: 600, color: colorBen }}>
            {fmtEur(beneficio, { showEuro: false, decimals: 2, signed: true })}
          </div>
      ...
      <div style={{ fontFamily: LEXEND, fontSize: 12, color: '#3a4050', marginTop: 10, marginBottom: 14 }}>
        Margen contribución <strong style={{ fontFamily: OSWALD, color: VERDE }}>{fmtPct(margenContribPct, 2)}</strong>
      </div>
        ...
        {filas.map((f, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontFamily: LEXEND, fontSize: 13 }}>
            ...
            <span style={{ fontFamily: OSWALD, fontWeight: 600, color: f.color }}>
              {fmtEur(f.valor, { showEuro: false, decimals: 2, signed: f.signed })}
            </span>
          </div>
        ))}
        <div style={{
        ...
        {filas.map((f, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{ fontFamily: OSWALD, fontSize: 12, color: '#7a8090', minWidth: 60, textAlign: 'right' }}>
                {fmtPct(pct, 2)}
              </span>
            </div>
          )
...
const PRESETS: Array<{ key: PresetKey; label: string; tipo: 'mejora'|'empeora'|'mixto'|'base' }> = [
  { key: 'base',           label: 'Datos reales',                    tipo: 'base' },
  ...
  { key: 'add-cocinero',   label: '+ Añadir cocinero (+1.800 fijos)', tipo: 'empeora' },
...
function aplicarPreset(preset: PresetKey, base: { ticketMedio: number; margenPct: number; fijos: number; pedidos: number }): Omit<Escenario,'id'|'preset'> {
  switch (preset) {
...
function TabSimulador(p: {
  ...
  const baseEscenario: Escenario = {
    id: 'base', preset: 'base',
    ticketMedio: ticketMedioBase, margenPct: p.margenContribPct,
    fijos: p.totalFijos, pedidosMes: p.totalPedidos, bloqueado: true,
  }

  const [escenarios, setEscenarios] = useState<Escenario[]>(() => [
    baseEscenario,
    { id: 'esc-1', preset: 'subir-ticket', ...aplicarPreset('subir-ticket', baseValores) },
    { id: 'esc-2', preset: 'add-cocinero', ...aplicarPreset('add-cocinero', baseValores) },
    { id: 'esc-3', preset: 'mas-pedidos',  ...aplicarPreset('mas-pedidos', baseValores) },
  ])
  ...
  function cambiarPreset(id: string, preset: PresetKey) {
    setEscenarios(prev => prev.map(e => e.id !== id ? e : { ...e, preset, ...aplicarPreset(preset, baseValores) }))
  }
  ...
  function añadirEscenario() {
    setEscenarios(prev => {
      ...
      return [...prev, { id: `esc-${Date.now()}`, preset: 'custom', ...baseValores, pedidosMes: baseValores.pedidos }]
    })
  }
  ...
  function calcularEscenario(e: Escenario) {
    ...
    const diasNec = peValor && p.brutoMedioDiario > 0 ? Math.ceil(peValor / p.brutoMedioDiario) : null
    const pedNec = peValor && e.ticketMedio > 0 ? Math.ceil(peValor / e.ticketMedio) : null
    const beneficio = peValor != null ? brutoEsperado - peValor : 0
    return { peValor, diasNec, pedNec, brutoEsperado, beneficio }
  }
    ...
    <div style={{ marginTop: 16 }}>
      <div style={{
        ...
        {escenarios.map(e => {
          ...
          return (
            <CardEscenario
              key={e.id} escenario={e} calc={calc} calcBase={calcBase}
              onChangePreset={(p) => cambiarPreset(e.id, p)}
              onChange={(c, v) => actualizarEscenario(e.id, c, v)}
        ...
        {escenarios.length < 6 && (
          <button onClick={añadirEscenario} style={{
            ...cardBig,
            border: '1.5px dashed #d0c8bc', cursor: 'pointer',
            display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'center',
            gap: 6, minHeight: 320, background: 'transparent',
            fontFamily: OSWALD, color: '#7a8090', fontSize: 14,
...
function CardEscenario({ escenario, calc, calcBase, onChangePreset, onChange, onDelete }: {
  escenario: Escenario
  calc: { peValor: number | null; diasNec: number | null; pedNec: number | null; brutoEsperado: number; beneficio: number }
  calcBase: { peValor: number | null; diasNec: number | null; pedNec: number | null; brutoEsperado: number; beneficio: number }
        ...
        <div style={{ flex: 1 }}>
          {e.bloqueado ? (
            ...
            <select
              ...
              style={{
                fontFamily: OSWALD, fontSize: 13, fontWeight: 600,
                letterSpacing: '0.5px', color: '#111111',
...
function RowInput({ label, value, decimales, onChange, bloqueado }: {
  ...
  const styleInput: CSSProperties = {
    ...
    border: '0.5px solid #d0c8bc', borderRadius: 6, fontSize: 13,
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 10, alignItems: 'center', padding: '5px 0' }}>
      ...
      <input
        type="number" step={decimales > 0 ? 0.01 : 1}
