...
/* Trimestre meses: opacidad .08, trimestre totals: .16, mes actual: .18 */
const QBG=['rgba(30,91,204,.08)','rgba(29,158,117,.08)','rgba(245,166,35,.08)','rgba(176,29,35,.08)']
const QHD=['rgba(30,91,204,.18)','rgba(29,158,117,.18)','rgba(245,166,35,.18)','rgba(176,29,35,.18)']
const QCL=['#1E5BCC','#1D9E75','#f5a623','#B01D23']
const CUR_BG='rgba(30,91,204,.18)'
...
useEffect(()=>{const main=scrollRef.current;const top=topScrollRef.current;if(!main||!top)return;let syncing=false;const sync=(src:HTMLDivElement,dst:HTMLDivElement)=>()=>{if(syncing)return;syncing=true;dst.scrollLeft=src.scrollLeft;syncing=false};const onMain=sync(main,top);const onTop=sync(top,main);main.addEventListener('scroll',onMain);top.addEventListener('scroll',onTop);return()=>{main.removeEventListener('scroll',onMain);top.removeEventListener('scroll',onTop)}},[])
...
const colBg=(c:Col):string|undefined=>{if(c.isCur&&!c.isQ&&!c.isY)return CUR_BG;const q=cQ(c);if(c.isQ)return QHD[q-1];if(c.isY)return`${COLORS.redSL}10`;if(q)return QBG[q-1];return undefined}
const thC=(c:Col):React.CSSProperties=>{const q=cQ(c);const qy=c.isQ||c.isY;return{fontFamily:FONT.heading,fontSize:12,fontWeight:qy?700:(c.isCur?700:500),letterSpacing:'1.5px',textTransform:'uppercase',textAlign:'right',padding:'4px 4px',borderBottom:`1px solid ${COLORS.brd}`,whiteSpace:'nowrap',position:'sticky',top:0,zIndex:2,cursor:c.isQ?'pointer':'default',userSelect:'none',color:qy?(c.isY?COLORS.redSL:QCL[q-1]):(c.isCur?'#fff':COLORS.mut),background:c.isCur&&!qy?BL:(qy?(c.isY?`${COLORS.redSL}14`:QHD[q-1]):(q?QBG[q-1]:COLORS.bg)),...(c.isY?{borderLeft:`4px solid ${COLORS.redSL}`,paddingLeft:8}:{})}}
...
const td0=(c:Col):React.CSSProperties=>{const qy=c.isQ||c.isY;return{padding:'1px 4px',fontSize:16,fontFamily:FONT.body,color:COLORS.sec,borderBottom:`0.5px solid ${COLORS.brd}18`,whiteSpace:'nowrap',textAlign:'right',verticalAlign:'middle',fontVariantNumeric:'tabular-nums',lineHeight:1.2,fontWeight:qy?600:(c.isCur?500:400),background:colBg(c),...(c.isY?{borderLeft:`4px solid ${COLORS.redSL}`,paddingLeft:8}:{})}}
...
{/* Scrollbar duplicada arriba - sincronizada */}
<div ref={topScrollRef} style={{overflowX:'auto',overflowY:'hidden',position:'sticky',top:0,zIndex:10,background:COLORS.bg,borderBottom:`1px solid ${COLORS.brd}40`}}><div style={{width:tW,height:1}}/></div>
...
<tr style={{background:RB}}><td style={rRow(RATIO_COLORS.margen)}>MARGEN BRUTO <span style={{fontSize:10,color:COLORS.mut,fontWeight:400}}>(Ingresos − Producto)</span></td><Cells fn={mB} pctFn={ms=>po(mB(ms),iM(ms))} sign vc={COLORS.ok} rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={rRow(RATIO_COLORS.food)}>FOOD COST % <span style={{fontSize:10,color:COLORS.mut,fontWeight:400}}>(Producto / Ingresos)</span></td><Cells fn={ms=>po(gP('2.1',ms),iM(ms))} pct rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={rRow(RATIO_COLORS.labor)}>LABOR COST % <span style={{fontSize:10,color:COLORS.mut,fontWeight:400}}>(Equipo / Ingresos)</span></td><Cells fn={ms=>po(gP('2.2',ms),iM(ms))} pct rz/></tr>
