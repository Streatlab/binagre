...
const cM=new Date().getMonth()+1,BLUE='#1E5BCC'
...
const P=3
const thC=(c:Col):React.CSSProperties=>{const q=cQ(c);const qy=c.isQ||c.isY;return{fontFamily:FONT.heading,fontSize:11,fontWeight:qy?700:500,letterSpacing:'1.5px',textTransform:'uppercase',textAlign:'right',padding:`6px ${P}px`,borderBottom:`0.5px solid ${COLORS.brd}`,whiteSpace:'nowrap',position:'sticky',top:0,zIndex:2,cursor:c.isQ?'pointer':'default',userSelect:'none',color:qy?(c.isY?COLORS.redSL:QCL[q-1]):COLORS.mut,background:qy?(c.isY?`${COLORS.redSL}14`:QHD[q-1]):(q?QBG[q-1]:COLORS.bg)}}
const th1:React.CSSProperties={fontFamily:FONT.heading,fontSize:11,fontWeight:600,letterSpacing:'1.5px',color:COLORS.mut,textTransform:'uppercase',textAlign:'left',padding:`6px 8px`,background:COLORS.bg,borderBottom:`0.5px solid ${COLORS.brd}`,whiteSpace:'nowrap',position:'sticky',left:0,top:0,zIndex:4,minWidth:190}
const thP=(c:Col):React.CSSProperties=>({...thC(c),fontSize:9,letterSpacing:'0.5px',color:COLORS.mut+'80',minWidth:30,padding:`6px 2px`})
const tdC=(c:Col):React.CSSProperties=>{const q=cQ(c);const qy=c.isQ||c.isY;return{padding:`2px ${P}px`,fontSize:15,fontFamily:FONT.body,color:COLORS.sec,borderBottom:`0.5px solid ${COLORS.brd}15`,whiteSpace:'nowrap',textAlign:'right',verticalAlign:'middle',fontVariantNumeric:'tabular-nums',lineHeight:1.25,fontWeight:qy?600:400,background:qy?(c.isY?`${COLORS.redSL}08`:QBG[q-1]):(q?QBG[q-1]:undefined)}}
const t1:React.CSSProperties={padding:`2px 8px`,fontSize:13,fontFamily:FONT.body,color:COLORS.sec,borderBottom:`0.5px solid ${COLORS.brd}15`,whiteSpace:'nowrap',textAlign:'left',position:'sticky',left:0,zIndex:1,verticalAlign:'middle',background:'#fff'}
const tdP=(c:Col):React.CSSProperties=>({...tdC(c),fontSize:11,color:COLORS.mut+'99',padding:`2px 2px`,minWidth:30})
const nZ=(c:Col,r?:boolean):React.CSSProperties=>({fontFamily:FONT.heading,fontSize:c.isY?(r?22:18):(c.isQ?(r?19:16):(c.isCur?(r?18:16):(r?17:15))),fontWeight:c.isY?700:(c.isQ||c.isCur?600:500),letterSpacing:'0.3px'})
const RB='#fdf6ed'
const r1=(bc?:string):React.CSSProperties=>({...t1,background:RB,borderLeft:`3px solid ${bc||COLORS.redSL}`,fontSize:14})
const rC=(c:Col):React.CSSProperties=>({...tdC(c),background:c.isY?`${COLORS.redSL}10`:(c.isQ?tdC(c).background:RB)})
const Cells=({fn,sign,pct,pctFn,alertMax,vc:vcl,rz,tip}:{fn:(ms:number[])=>number;sign?:boolean;pct?:boolean;pctFn?:(ms:number[])=>number;alertMax?:number;vc?:string;rz?:boolean;tip?:string})=>(<>{vc.map((c,i)=>{const v=fn(c.ms);const cl=vcl||(sign?(v>0?COLORS.ok:v<0?COLORS.err:COLORS.mut):undefined);const pr=sign&&v>0?'+':'';const st=rz?rC(c):tdC(c);const vt=<td key={i} style={{...st,color:cl||st.color,...(v&&!pct?nZ(c,rz):{}),fontFamily:v&&!pct?FONT.heading:st.fontFamily}}>{pct?fP(v):v?`${pr}${fI(v)}`:'—'}</td>;if(pctFn){const pv=pctFn(c.ms);const ov=alertMax&&pv>alertMax;return[vt,<td key={`p${i}`} style={{...tdP(c),color:ov?COLORS.err:undefined,fontWeight:ov?600:undefined,background:rz?RB:tdP(c).background}} title={tip}>{pv?(ov?<span style={{display:'inline-flex',alignItems:'center',gap:1}}><span style={{fontSize:11,animation:'pulse 2s infinite'}}>⚠</span>{pv.toFixed(1)}%</span>:`${pv.toFixed(1)}%`):'—'}</td>]}return vt})}</>)
const CTM=({rz:r}:{rz?:boolean})=>(<>{vc.map((c,i)=>{const p=pe(c.ms);const tb=tB(c.ms);const tn=tN(c.ms);const st=r?rC(c):tdC(c);const sz=c.isY?18:(c.isQ?16:15);return[<td key={i} style={{...st,fontFamily:FONT.heading,fontSize:sz,fontWeight:c.isY?700:(c.isQ?600:500),letterSpacing:'0.3px'}}>{p?<><span style={{color:BLUE}}>{fI(p)}</span>{' '}<span style={{color:COLORS.warn,fontSize:sz}}>{fD(tb)}</span><span style={{color:COLORS.mut,fontSize:sz-4}}>/</span><span style={{color:COLORS.ok,fontSize:sz}}>{fD(tn)}</span></>:'—'}</td>,<td key={`p${i}`} style={{...tdP(c),background:r?RB:tdP(c).background}}/>]})}</>)
const CI=({rz:r}:{rz?:boolean})=>(<>{vc.map((c,i)=>{const rv=iT(c.ms);const e=nE(c.ms);const v=rv||e;const es=!rv&&e>0;const st=r?rC(c):tdC(c);return[<td key={i} style={{...st,...nZ(c,r),fontStyle:es?'italic':undefined,color:es?COLORS.ok:undefined}}>{v?fI(v):'—'}{es&&<span style={{fontSize:9,color:COLORS.mut,marginLeft:2}}>(est.)</span>}</td>,<td key={`p${i}`} style={{...tdP(c),background:r?RB:tdP(c).background}}/>]})}</>)
const CR=({fn,rz:r}:{fn:(ms:number[])=>number;rz?:boolean})=>(<>{vc.map((c,i)=>{const v=fn(c.ms);const st=r?rC(c):tdC(c);return[<td key={i} style={{...st,fontSize:12,color:COLORS.mut,fontStyle:'italic',fontFamily:FONT.heading}}>{v?fD(v):'—'}</td>,<td key={`p${i}`} style={{...tdP(c),background:r?RB:tdP(c).background}}/>]})}</>)
const CB=({fn,max}:{fn:(ms:number[])=>number;max:number})=>(<>{vc.map((c,i)=>{const v=fn(c.ms);const bc=!v?COLORS.err:(v<=max*.5?COLORS.ok:(v<=max?COLORS.warn:COLORS.err));const bw=v?Math.min(v/max*100,100):100;return[<td key={i} style={{...tdC(c),...nZ(c),color:v>max?COLORS.err:undefined}}>{v?fP(v):'—'}<div style={{display:'block',width:'100%',height:5,borderRadius:3,background:COLORS.err+'30',marginTop:2,overflow:'hidden'}}><div style={{height:'100%',borderRadius:3,background:bc,width:`${bw}%`}}/></div></td>,<td key={`p${i}`} style={tdP(c)}/>]})}</>)
...
const sH=(l:string,cl:string,x?:React.ReactNode)=><tr><td colSpan={99} style={{...t1,background:COLORS.bg,fontFamily:FONT.heading,fontSize:12,letterSpacing:'2px',textTransform:'uppercase',color:cl,padding:'10px 8px 4px',borderBottom:`1.5px solid ${cl}40`,borderLeft:`3px solid ${cl}`,fontWeight:700}}>{l}{x}</td></tr>
...
const sp=<tr><td colSpan={99} style={{height:8,border:'none',background:COLORS.bg,padding:0}}/></tr>
...
<tr style={{background:RB}} {...hv}><td style={{...r1(),fontFamily:FONT.heading,fontSize:13,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.sec,fontWeight:500}}>Facturación bruta</td><Cells fn={fB} rz/></tr>
<tr style={{background:RB}}><td style={{...r1(),fontFamily:FONT.heading,fontSize:14,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.ok,fontWeight:700}}>Ingresos netos</td><CI rz/></tr>
<tr style={{background:RB}} {...hv}><td style={{...r1(),color:COLORS.mut}}><span style={{color:BLUE,fontWeight:600}}>Pedidos</span> · <span style={{color:COLORS.warn}}>TM Bruto</span> / <span style={{color:COLORS.ok}}>TM Neto</span></td><CTM rz/></tr>
...
<tr style={{background:RB}} {...hv}><td style={{...r1(COLORS.err),fontFamily:FONT.heading,fontSize:13,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.sec,fontWeight:600}}>Gastos fijos</td><Cells fn={gF} pctFn={ms=>po(gF(ms),fB(ms))} rz tip="% sobre Facturación bruta"/></tr>
<tr style={{background:RB}}><td style={{...r1(COLORS.err),paddingLeft:20,fontSize:12,color:COLORS.mut,fontStyle:'italic'}}>por pedido</td><CR fn={ms=>{const p=pe(ms);return p?gF(ms)/p:0}} rz/></tr>
<tr style={{background:RB}} {...hv}><td style={{...r1(COLORS.err),fontFamily:FONT.heading,fontSize:13,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.sec,fontWeight:600}}>Gastos variables</td><Cells fn={gV} pctFn={ms=>po(gV(ms),fB(ms))} rz tip="% sobre Facturación bruta"/></tr>
<tr style={{background:RB}}><td style={{...r1(COLORS.err),paddingLeft:20,fontSize:12,color:COLORS.mut,fontStyle:'italic'}}>por pedido</td><CR fn={ms=>{const p=pe(ms);return p?gV(ms)/p:0}} rz/></tr>
...
<tr style={{background:RB}}><td style={{...r1(COLORS.ok),fontFamily:FONT.heading,fontSize:14,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.ok,fontWeight:700}}><span style={{display:'inline-block',width:8,height:8,borderRadius:'50%',background:COLORS.ok,marginRight:6,verticalAlign:'middle'}}/>Margen bruto</td><Cells fn={mB} pctFn={ms=>po(mB(ms),iM(ms))} sign vc={COLORS.ok} rz/></tr>
<tr><td style={{...t1,fontFamily:FONT.heading,fontSize:12,letterSpacing:'1.5px',textTransform:'uppercase',color:BLUE,fontWeight:600,background:'rgba(30,91,204,.04)',borderTop:'2px dashed #1E5BCC',borderLeft:`3px solid ${BLUE}`}}>Break-even</td><Cells fn={gF}/></tr>
<tr style={{background:RB}} {...hv}><td style={{...r1(COLORS.ok),fontFamily:FONT.heading,fontSize:12,letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700}}>Coste directo <span style={{color:COLORS.mut,fontSize:10,fontWeight:400}}>(Producto + Equipo · obj &lt;60%)</span></td><Cells fn={cD} pct rz alertMax={60}/></tr>
<tr><td colSpan={99} style={{height:16,border:'none',background:COLORS.bg,padding:0}}/></tr>
{sH('Detalle por categoría',COLORS.mut,<button onClick={()=>{const n=!aO;const nv:Record<string,boolean>={};['1','2.1','2.2','2.3','2.4'].forEach(k=>nv[k]=n);sD(nv);sAO(n)}} style={{float:'right',padding:'3px 10px',borderRadius:5,border:`0.5px solid ${COLORS.brd}`,background:COLORS.card,fontFamily:FONT.body,fontSize:11,color:COLORS.mut,cursor:'pointer'}}>{aO?'▴ Colapsar':'▾ Expandir'}</button>)}{rA() as any}
<tr style={{cursor:'pointer'}} onClick={()=>sD(p=>({...p,'1':!p['1']}))}><td style={gR}>{det['1']?'▾':'▸'} 1. Ingresos</td><Cells fn={iM}/></tr>
{det['1']&&ingC.map(c=>{const b=aB();return<tr key={c.id} style={{background:b}} {...hv}><td style={{...t1,paddingLeft:24}}>{c.nombre}</td><Cells fn={ms=>sumMeses(ingresos[c.id]||{},ms)} pctFn={ms=>po(sumMeses(ingresos[c.id]||{},ms),iM(ms))}/></tr>})}{sp}
...
...(op?sN.flatMap(sub=>{const ch=cCh(sub.id);return[<tr key={sub.id} style={{background:aB()}} {...hv}><td style={{...t1,paddingLeft:24,fontWeight:500}}>{sub.nombre}</td><Cells fn={ms=>sumCatMeses(gastos,sub.id,ms)} pctFn={ms=>po(sumCatMeses(gastos,sub.id,ms),iM(ms))}/></tr>,...ch.map(c=><tr key={c.id} style={{background:aB()}} {...hv}><td style={{...t1,paddingLeft:36,color:COLORS.mut,fontSize:12}}>{c.nombre}</td><Cells fn={ms=>sumMeses(gastos[c.id]||{},ms)} pctFn={ms=>po(sumMeses(gastos[c.id]||{},ms),iM(ms))}/></tr>)].filter(Boolean)}):[]),
