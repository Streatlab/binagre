...
/* ── Estilos compactos ── */
const thS=(c:Col):React.CSSProperties=>{const q=cQ(c);const qy=c.isQ||c.isY;return{fontFamily:FONT.heading,fontSize:10,fontWeight:qy?700:500,letterSpacing:'1.5px',textTransform:'uppercase' as const,textAlign:'right' as const,padding:'8px 4px',borderBottom:`0.5px solid ${COLORS.brd}`,whiteSpace:'nowrap' as const,position:'sticky' as const,top:0,zIndex:c.isY?5:2,cursor:c.isQ?'pointer':'default',userSelect:'none' as const,color:qy?(c.isY?'#fff':QCL[q-1]):COLORS.mut,background:qy?(c.isY?COLORS.redSL:QHD[q-1]):(q?QBG[q-1]:COLORS.bg),right:c.isY?0:undefined}}
const th1:React.CSSProperties={fontFamily:FONT.heading,fontSize:10,fontWeight:500,letterSpacing:'1.5px',color:COLORS.mut,textTransform:'uppercase',textAlign:'left',padding:'8px 10px',background:COLORS.bg,borderBottom:`0.5px solid ${COLORS.brd}`,whiteSpace:'nowrap',position:'sticky',left:0,top:0,zIndex:6,width:200,minWidth:180}
const thP=(c:Col):React.CSSProperties=>({...thS(c),fontSize:8,letterSpacing:'0.5px',color:c.isY?'#ffffff90':COLORS.mut+'80',minWidth:28,padding:'8px 2px'})
const tdc=(c:Col):React.CSSProperties=>{const q=cQ(c);const qy=c.isQ||c.isY;return{padding:'4px 4px',fontSize:13,fontFamily:FONT.body,color:COLORS.sec,borderBottom:`0.5px solid ${COLORS.brd}20`,whiteSpace:'nowrap',textAlign:'right',verticalAlign:'middle',fontVariantNumeric:'tabular-nums',lineHeight:1.3,fontWeight:qy?600:400,background:qy?(c.isY?`${COLORS.redSL}08`:QBG[q-1]):(q?QBG[q-1]:undefined),right:c.isY?0:undefined,position:c.isY?'sticky' as const:undefined,zIndex:c.isY?3:undefined}}
...
const tdP=(c:Col):React.CSSProperties=>({...tdc(c),fontSize:9,color:c.isY?COLORS.redSL+'90':COLORS.mut+'99',padding:'4px 2px',minWidth:28})
/* Tamaños numéricos: resumen más grande */
...
/* TM: pedidos en negro, TMB naranja, TMN verde — TODOS mismo tamaño 14px */
const CTM=({rz:r}:{rz?:boolean})=>(<>{vc.map((c,i)=>{const p=ped(c.ms);const tb=tB(c.ms);const tn=tN(c.ms);const st=r?rC(c):tdc(c);const sz=c.isY?16:(c.isQ?15:14);return[<td key={i} style={{...st,fontFamily:FONT.heading,fontSize:sz,fontWeight:c.isY?700:500,letterSpacing:'0.3px'}}>{p?<><span style={{color:COLORS.pri}}>{fmt(p)}</span>{' '}<span style={{color:COLORS.warn}}>{f2(tb)}</span><span style={{color:COLORS.mut,fontSize:10}}> / </span><span style={{color:COLORS.ok}}>{f2(tn)}</span></>:'—'}</td>,<td key={`p${i}`} style={{...tdP(c),background:r?rBg:tdP(c).background}}/>]})}</>)
...
{sep}
{/* ═══ DETALLE POR CATEGORÍA (siempre visible) ═══ */}
...
{det['1']&&ingC.map(c=>{if(!vi(c.nombre))return null;const b=aB();return<tr key={c.id} style={{background:b}} {...hv}><td style={{...td1,paddingLeft:24,fontSize:12}}>{c.nombre}</td><Cells fn={ms=>sumMeses(ingresos[c.id]||{},ms)} pctFn={ms=>po(sumMeses(ingresos[c.id]||{},ms),iM(ms))}/></tr>})}
{sep}
{/* Grupos de gastos */}
...
...ch.map(c=>{if(!vi(c.nombre))return null;return<tr key={c.id} style={{background:aB()}} {...hv}><td style={{...td1,paddingLeft:36,color:COLORS.mut,fontSize:11}}>{c.nombre}</td><Cells fn={ms=>sumMeses(gastos[c.id]||{},ms)} pctFn={ms=>po(sumMeses(gastos[c.id]||{},ms),iM(ms))}/></tr>})].filter(Boolean)}):[]),
