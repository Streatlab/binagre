...
import {
  periodoMesActual, CATEGORIAS_ORDEN, statusRango,
} from '@/lib/running';
  ...
  if (error) {
    return (
      <div style={{ padding: 24, background: 'var(--rf-bg-page)', minHeight: '100vh' }}>
        <div style={{ background:'var(--rf-red-soft)', border:'1px solid var(--rf-red)', color:'var(--rf-red)', padding:16, borderRadius:8 }}>
          Error: {error}
        </div>
      </div>
    );
    ...
    <div style={{ background: 'var(--rf-bg-page)', minHeight: '100vh', padding: '24px 32px' }}>
      <div style={{ display:'flex', alignItems:'baseline', gap:12, marginBottom:24, flexWrap:'wrap' }}>
        <span style={{
          ...
          color:'var(--rf-red)', letterSpacing:'0.1em', textTransform:'uppercase',
        }}>Running financiero</span>
      ...
      <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:24, flexWrap:'wrap' }}>
        ...
        <button onClick={() => setModalOpen(true)} style={{
          marginLeft:'auto',
          background:'var(--rf-red)', color:'#fff', border:'none',
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:16, marginBottom:20 }} className="rf-kpi-row">
        <KpiCard
          ...
          sub={`Neto recibido ${fmtEur(totalNeto)}`}
          subVariant="neutral"
        />
        <KpiCard
          label="Comisiones plataformas"
          value={fmtEur(comisiones)}
          valueColor="var(--rf-red)"
          sub={`${totalBruto ? ((comisiones/totalBruto)*100).toFixed(1) : '0'}% sobre bruto`}
          subVariant="warn"
        />
        <KpiCard
          label="Total gastos"
          value={fmtEur(totalGasto)}
          valueColor="var(--rf-orange)"
          sub={`${ratio.toFixed(1)}% sobre bruto`}
          subVariant="warn"
        />
        <KpiCard
          label="Resultado"
          value={(resultado >= 0 ? '+' : '−') + fmtEur(Math.abs(resultado)).replace('−','')}
          valueColor={resultado >= 0 ? 'var(--rf-green)' : 'var(--rf-red)'}
          sub={`${resultado >= 0 ? '+' : ''}${pctResultado}% sobre neto`}
          subVariant={resultado >= 0 ? 'up' : 'down'}
        />
      </div>
      ...
      <div style={{
        ...
        color:'var(--rf-text)', fontWeight:600, marginBottom:14, textTransform:'uppercase',
      }}>PyG detallado · {anio}</div>
