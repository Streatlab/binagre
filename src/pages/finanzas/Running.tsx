...
export default function Running() {
  ...
  const { ingresos, gastos, brutos, categorias, benchmarks, comisiones, loading } = useRunningAnual(anio, tId)
  const COM = { uber:comisiones.uber||0, glovo:comisiones.glovo||0, je:comisiones.je||0, web:comisiones.web||0, directa:comisiones.directa||0 }

  useEffect(() => { (async()=>{ const{data}=await supabase.from('resumenes_plataforma_marca_mensual').select('*').eq('año',anio); sRes((data||[])as ResRow[]) })() }, [anio])
  ...
  const reembolsos2xUberMes = useMemo(() => {
    const m: Record<number, number> = {}
    for (const r of resumenes) {
      if (r.plataforma === 'uber' && r.reembolsos_2x) {
        m[r.mes] = (m[r.mes] || 0) + Number(r.reembolsos_2x || 0)
      }
    }
    return m
  }, [resumenes])
  ...
  const CI = ({rz:r}:{rz?:boolean}) => (<>{vc.map((c,i)=>{ const rv=iT(c.ms); const e=nE(c.ms); const extra=reembolsos2xMs(c.ms); const v=(rv||e)+extra; const es=!rv && e>0; const st=r?rC(c):td0(c); return [<td key={i} style={{...st, ...nZ(c,r), color:COLORS.ok, fontStyle:es?'italic':undefined}}>{v?fI(v):'—'}{es && <span style={{fontSize:9,color:COLORS.mut,marginLeft:2,fontStyle:'normal',fontFamily:FONT.body}} title="Estimado: bruto × (1 − comisión)">(est.)</span>}</td>, <td key={`p${i}`} style={tdP(c,r)}/>] })}</>)
  ...
  return (
    <div style={{background:COLORS.bg, padding:'20px 24px', minHeight:'100vh'}}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10, flexWrap:'wrap', gap:6}}>
        <h2 style={{color:COLORS.redSL, fontFamily:FONT.heading, fontSize:22, fontWeight:600, letterSpacing:'3px', margin:0, textTransform:'uppercase'}}>RUNNING {anio}</h2>
        <div style={{display:'flex', gap:4, alignItems:'center'}}>
          ...
          <div style={{...TABS_PILL.container}}><select value={anio} onChange={e=>sA(Number(e.target.value))} style={pS}>{[2026,2025,2024].map(a=><option key={a} value={a}>{a}</option>)}</select></div>
      ...
      <div style={{...CARDS.std, padding:0, overflow:'visible'}}>
        <div ref={mainRef} style={{overflowX:'auto'}}>
          <table style={{width:'100%', borderCollapse:'separate', borderSpacing:0, minWidth:tW}}>
            <thead>
              <tr>
                <th style={th1}>PyG <button onClick={togglePyGAll} style={{...btnSL, marginLeft:8}}>{allBl?'▴ Colapsar todo':'▾ Expandir todo'}</button></th>
                {vc.map((c,i)=>[<th key={i} style={thC(c)} onClick={c.qn?()=>tQ(c.qn!):undefined}>{c.label}</th>, <th key={`p${i}`} style={thP(c)}>%</th>])}
              </tr>
            </thead>
            <tbody>
            ...
            {sHBl('Distribución de gastos · % s/Ingresos netos', 'dist', COLORS.warn)}{rA() as any}
            {bloque.dist && <>
            ...
            {sH('Detalle por categoría', COLORS.mut, <button onClick={(e)=>{e.stopPropagation(); toggleDetAll()}} style={{...btnSL, marginLeft:10}}>{aO?'▴ Colapsar todo':'▾ Expandir todo'}</button>)}{rA() as any}
            {detHeaders}
            ...
            {det['1'] && <>
              ...
              <tr style={{background:aB()}} {...hv}><td style={{...t1, paddingLeft:38, color:COLORS.ok+'cc', fontSize:13, fontStyle:'italic'}}>1.1.1.1 · Reembolsos Uber 2x <span style={{fontSize:9, color:COLORS.mut, fontStyle:'normal'}}>(de resúmenes OCR)</span></td><Cells fn={reembolsos2xMs} vc={COLORS.ok}/></tr>
