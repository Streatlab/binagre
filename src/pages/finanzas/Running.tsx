...
/* Formato números con separador de miles ES */
...
export default function Running(){
  ...
  /* Cells con columna % s/IN intercalada */
  ...
  /* Gastos fijos = 2.2 RRHH + 2.3 Alquiler. Variables = 2.1 Producto + 2.4 Controlables */
  ...
  /* Header: 2 filas — mes + % s/IN */
  const visibleCols = cols.filter(c=>!c.hid)
  const headerCols = visibleCols.flatMap(c=>[c, {...c, label:'% s/IN', _isPct:true} as any])
      ...
      <div style={{background:'#fff',border:'0.5px solid #d0c8bc',borderRadius:14,overflow:'hidden'}}>
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'separate',borderSpacing:0,fontFamily:'Lexend, sans-serif',fontSize:13,minWidth:1400}}>
            ...
            <tbody>
              {/* ══════ BLOQUE RESUMEN (estilo Excel) ══════ */}
              {secH('Resumen')}
              {rA() as any}
              {(()=>{const b=aB();return<tr style={{background:b}} {...hv}><td style={grpS}>Ingresos</td><Cells fn={iT} bg='rgba(176,29,35,0.04)'/></tr>})()}
              ...
              {/* ══════ DISTRIBUCIÓN DE GASTOS (% s/IN) ══════ */}
              {secH('Distribución de gastos')}
              {rA() as any}
              {grupos.map(g=>{const bn=gB(g.id);const sc=sC(pctOf(gP(g.id,ALL),iT(ALL)),bn);const bI=bn?` (${bn.pct_min}-${bn.pct_max}%)`:'';const b=aB();return<tr key={g.id} style={{background:b}} {...hv}><td style={grpS}><span style={{display:'inline-block',width:7,height:7,borderRadius:'50%',background:sc,marginRight:5,verticalAlign:'middle'}}/>{g.id} {g.nombre}<span style={{color:'#7a8090',fontSize:10,fontWeight:400,letterSpacing:'1px'}}>{bI}</span></td><Cells fn={ms=>gP(g.id,ms)} pctFn={ms=>pctOf(gP(g.id,ms),iT(ms))} bg='rgba(176,29,35,0.04)'/></tr>})}
              <tr><td style={totS}>Total gastos</td><Cells fn={gT} pctFn={ms=>pctOf(gT(ms),iT(ms))} bg='rgba(176,29,35,0.07)'/></tr>
              <tr><td style={resS}>Beneficio</td><Cells fn={res} sign pctFn={ms=>pctOf(res(ms),iT(ms))} bg='rgba(29,158,117,0.06)'/></tr>
