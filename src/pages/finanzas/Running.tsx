...
const sC=(p:number,b:{pct_min:number;pct_max:number}|null|undefined)=>{if(!b)return COLORS.mut;if(p<=b.pct_max)return COLORS.ok;if(p<=b.pct_max*1.5)return COLORS.warn;return COLORS.err}
...
const thC=(c:Col):React.CSSProperties=>{const q=cQ(c);const qy=c.isQ||c.isY;return{fontFamily:FONT.heading,fontSize:12,fontWeight:qy?700:500,letterSpacing:'1.5px',textTransform:'uppercase',textAlign:'right',padding:'4px 1px',borderBottom:`1px solid ${COLORS.brd}`,whiteSpace:'nowrap',position:'sticky',top:0,zIndex:2,cursor:c.isQ?'pointer':'default',userSelect:'none',color:qy?(c.isY?COLORS.redSL:QCL[q-1]):COLORS.mut,background:qy?(c.isY?`${COLORS.redSL}14`:QHD[q-1]):(q?QBG[q-1]:COLORS.bg)}}
const th1:React.CSSProperties={fontFamily:FONT.heading,fontSize:12,fontWeight:600,letterSpacing:'1.5px',color:COLORS.mut,textTransform:'uppercase',textAlign:'left',padding:'4px 5px',background:COLORS.bg,borderBottom:`1px solid ${COLORS.brd}`,whiteSpace:'nowrap',position:'sticky',left:0,top:0,zIndex:4,minWidth:210}
...
/* Cells SIN pctFn por defecto — solo muestra % si pctFn está definido */
...
/* Barras semáforo INVERTIDO: verde=dentro benchmark, amarillo=exceso, rojo=resto */
const CB=({fn,max}:{fn:(ms:number[])=>number;max:number})=>(<>{vc.map((c,i)=>{const v=fn(c.ms);const bc=!v?COLORS.err:(v<=max?COLORS.ok:(v<=max*1.5?COLORS.warn:COLORS.err));const bw=v?Math.min(v/max*100,100):100;return[<td key={i} style={{...td0(c),...nZ(c),color:v>max?COLORS.err:(v<=max?COLORS.ok:undefined)}}>{v?fP(v):'—'}<div style={{width:'100%',height:5,borderRadius:3,background:COLORS.err+'30',marginTop:1,overflow:'hidden'}}><div style={{height:'100%',borderRadius:3,background:bc,width:`${bw}%`}}/></div></td>,<td key={`p${i}`} style={tdP(c)}/>]})}</>)
...
<tr style={{background:RB}} {...hv}><td style={{...r1(COLORS.err),fontFamily:FONT.heading,fontSize:15,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.sec,fontWeight:600}}>Gastos fijos</td><Cells fn={gF} pctFn={ms=>po(gF(ms),fB(ms))} rz tip="% sobre Facturación bruta"/></tr>
<tr style={{background:RB}} {...hv}><td style={{...r1(COLORS.err),fontFamily:FONT.heading,fontSize:15,letterSpacing:'1.5px',textTransform:'uppercase',color:COLORS.sec,fontWeight:600}}>Gastos variables</td><Cells fn={gV} pctFn={ms=>po(gV(ms),fB(ms))} rz tip="% sobre Facturación bruta"/></tr>
...
{sp}
<tr><td style={{...rR,background:RB,borderLeft:`3px solid ${COLORS.ok}`,fontSize:17}}>Resultado <span style={{fontSize:11,color:COLORS.mut,fontWeight:400}}>(Ingresos − Total gastos)</span> <Sp fn={re}/></td><Cells fn={re} sign rz/></tr>
{sp}
...
<tr style={{background:RB}}><td style={{...r1(COLORS.ok),fontFamily:FONT.heading,fontSize:15,textTransform:'uppercase',color:COLORS.ok,fontWeight:700}}>Margen bruto <span style={{fontSize:10,color:COLORS.mut,fontWeight:400}}>(Ingresos netos − Producto)</span></td><Cells fn={mB} pctFn={ms=>po(mB(ms),iM(ms))} sign vc={COLORS.ok} rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={{...r1(COLORS.ok),fontSize:14,color:COLORS.sec}}>Food Cost % <span style={{fontSize:10,color:COLORS.mut}}>(Producto / Ingresos netos)</span></td><Cells fn={ms=>po(gP('2.1',ms),iM(ms))} pct rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={{...r1(COLORS.ok),fontSize:14,color:COLORS.sec}}>Labor Cost % <span style={{fontSize:10,color:COLORS.mut}}>(Equipo / Ingresos netos)</span></td><Cells fn={ms=>po(gP('2.2',ms),iM(ms))} pct rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={{...r1(COLORS.ok),fontSize:14,color:COLORS.sec}}>Ratio Ingresos/Gastos <span style={{fontSize:10,color:COLORS.mut}}>(€ ingresado por € gastado)</span></td><Cells fn={ms=>{const g=gT(ms);return g?iM(ms)/g:0}} rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={{...r1(COLORS.ok),fontSize:14,color:COLORS.sec}}>Coste por pedido <span style={{fontSize:10,color:COLORS.mut}}>(Total gastos / Pedidos)</span></td><CR fn={ms=>{const p=pe(ms);return p?gT(ms)/p:0}} rz/></tr>
<tr style={{background:aB()}} {...hv}><td style={{...r1(COLORS.ok),fontSize:14,color:COLORS.sec}}>Margen neto por pedido <span style={{fontSize:10,color:COLORS.mut}}>(Resultado / Pedidos)</span></td><CR fn={ms=>{const p=pe(ms);return p?re(ms)/p:0}} rz/></tr>
...
<tr key={`st-${sub.id}`} style={{background:'#fafaf8'}}><td style={{...t1,paddingLeft:14,fontWeight:600,color:COLORS.mut,fontSize:13}}>Subtotal {sub.nombre}</td><Cells fn={ms=>sumCatMeses(gastos,sub.id,ms)}/></tr>
].filter(Boolean)}):[]),
