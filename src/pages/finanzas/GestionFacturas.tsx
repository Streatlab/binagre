...
interface FacturaRow {
  id: string; fecha_factura: string | null; proveedor_nombre: string; total: number | null
  estado: string | null; titular_id: string | null; pdf_drive_url: string | null
  pdf_filename: string | null; pdf_original_name: string | null; categoria_factura: string | null
  nif_emisor: string | null; tipo: string | null
}
...
function trimestreEnCurso(m: number): number { return Math.ceil(m / 3) }
...
function generarMeses(primeraFecha: string): MesSel[] {
  ...
  const result: MesSel[] = []
  let anio = hoy.getFullYear(); let mes = hoy.getMonth() + 1
  while (anio > ay || (anio === ay && mes >= am)) {
    result.push({ anio, mes }); mes--; if (mes === 0) { mes = 12; anio-- }
  }
  return result
}

function mesLabel(m: MesSel): string { return `${MESES_NOMBRE[m.mes]} ${m.anio}` }
function mesDesde(m: MesSel): string { return `${m.anio}-${String(m.mes).padStart(2,'0')}-01` }
...
function mesAnterior(): MesSel {
  const h = new Date(); let m = h.getMonth(); let a = h.getFullYear()
  if (m === 0) { m = 12; a-- }
  return { anio: a, mes: m }
}

function SelectorMes({ meses, seleccionado, onChange }: { meses: MesSel[]; seleccionado: MesSel; onChange: (m: MesSel) => void }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(o => !o)} style={{ padding: '6px 10px', borderRadius: 8, border: '0.5px solid #d0c8bc', background: '#ffffff', fontSize: 13, fontFamily: 'Lexend, sans-serif', color: '#111111', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}>
        <span>{mesLabel(seleccionado)}</span>
        <ChevronDown size={11} strokeWidth={2.5} style={{ marginLeft: 4 }} />
      </button>
      {open && (
        <div style={{ position: 'absolute', top: '100%', right: 0, background: '#fff', border: '0.5px solid #d0c8bc', borderRadius: 8, width: 200, fontSize: 13, boxShadow: '0 4px 12px rgba(0,0,0,0.06)', zIndex: 50, maxHeight: 280, overflowY: 'auto' }}>
          {meses.map((m, i) => {
            const isActive = m.anio === seleccionado.anio && m.mes === seleccionado.mes
            return (
              <button key={i} onClick={() => { onChange(m); setOpen(false) }} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', background: isActive ? '#FF475715' : 'transparent', color: isActive ? '#FF4757' : '#7a8090', border: 'none', fontSize: 13, fontFamily: 'Lexend, sans-serif', cursor: 'pointer', fontWeight: isActive ? 500 : 400 }}>
                {mesLabel(m)}
              </button>
            )
          })}
...
function buildDriveTree(facturas: FacturaRow[], titulares: Titular[]): DriveNode[] {
  ...
  for (const f of facturas) {
    ...
    const node = aMap.get(mes)!; node.count++; node.importe += Number(f.total || 0)
  }
    ...
    for (const anio of anios) {
      ...
      for (const trim of [1,2,3,4]) {
        ...
        for (const mes of MESES_POR_TRIM[trim]) {
          ...
          qNode.children!.push({ label: MESES_NOMBRE[mes], count: data.count, importe: data.importe, filtro: { titular_id: t.id, anio, trimestre: trim, mes }, kind: 'mes', trimNum: trim })
...
function flattenCategorias(cats: CategoriaPyg[]): Array<{ id: string; label: string }> {
  ...
  function walk(parent: string | null, depth: number) {
    for (const c of (byParent.get(parent) || [])) {
      out.push({ id: c.id, label: `${'   '.repeat(depth)}${c.id} ${c.nombre}` }); walk(c.id, depth+1)
    }
  }
  ...
  return (
    <div style={{ display: 'flex', gap: 5 }}>
      {(['ruben','emilio'] as const).map(t => {
        ...
        return (
          <button key={t} onClick={() => onChange(t)} style={{ padding: '8px 18px', borderRadius: 8, border: isActive ? 'none' : `0.5px solid ${COLORS.brd}`, background: isActive ? (t==='ruben' ? COLOR_RUBEN : COLOR_EMILIO) : '#fff', fontFamily: FONT.body, fontSize: 13, color: isActive ? '#fff' : '#3a4050', cursor: 'pointer', fontWeight: 500, minWidth: 90 }}>
...
export default function GestionFacturas() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [tRes, cRes, fRes, minRes] = await Promise.all([
        ...
        supabase.from('facturas').select('id, fecha_factura, proveedor_nombre, total, estado, titular_id, pdf_drive_url, pdf_filename, pdf_original_name, categoria_factura, nif_emisor, tipo').order('fecha_factura', { ascending: false, nullsFirst: false }),
      ...
      if (minRes.data?.[0]?.fecha_factura) setPrimeraFecha(minRes.data[0].fecha_factura)
      setLoading(false)
    }
  ...
  const plazoDate = new Date(mesSel.anio, mesSel.mes, 5)
  const hoyNow = new Date()
  const diasPlazo = Math.max(0, Math.ceil((plazoDate.getTime() - hoyNow.getTime()) / (1000*60*60*24)))
  const mesNext = mesSel.mes === 12 ? 1 : mesSel.mes + 1
  const plazoStr = `5 de ${MESES_NOMBRE[mesNext].toLowerCase()}`

  const titularActivo = useMemo(() => titulares.find(t => { const n = t.nombre.toLowerCase(); return titularKey==='ruben' ? n.includes('rubén')||n.includes('ruben') : n.includes('emilio') }) ?? null, [titulares, titularKey])
  const driveTreeFull = useMemo(() => buildDriveTree(facturas, titulares), [facturas, titulares])
  const driveTree = useMemo(() => titularActivo ? driveTreeFull.filter(t => t.filtro.titular_id === titularActivo.id) : [], [driveTreeFull, titularActivo])
  ...
  useEffect(() => {
    ...
    for (const t of driveTreeFull) {
      ...
      init[`t:${titId}`] = true; init[`y:${titId}:${anioActual}`] = true; init[`q:${titId}:${anioActual}:${trimActual}`] = true
    }
  ...
  const facturasFiltradas = useMemo(() => facturas.filter(f => {
    ...
    if (driveFiltro.anio && f.fecha_factura) {
      ...
      if (driveFiltro.trimestre && Math.ceil((d.getMonth()+1)/3) !== driveFiltro.trimestre) return false
      if (driveFiltro.mes && d.getMonth()+1 !== driveFiltro.mes) return false
    }
    if (categoriaId !== 'todas') { if (!f.categoria_factura) return false; if (!f.categoria_factura.startsWith(categoriaId)) return false }
    if (busqueda) { const q = busqueda.toLowerCase(); const hit = (f.proveedor_nombre||'').toLowerCase().includes(q)||(f.nif_emisor||'').toLowerCase().includes(q)||String(f.total||'').includes(q); if (!hit) return false }
    return true
  }), [facturas, titularActivo, driveFiltro, categoriaId, busqueda])

  const facturasOrdenadas = useMemo(() => {
    const arr = [...facturasFiltradas]; const mul = sortDir==='asc' ? 1 : -1
    arr.sort((a,b) => {
      ...
      switch (sortColumn) {
        ...
        case 'titular':   va = (titulares.find(t=>t.id===a.titular_id)?.nombre||'').toLowerCase(); vb = (titulares.find(t=>t.id===b.titular_id)?.nombre||'').toLowerCase(); break
  ...
  const facturasMesExportar = useMemo(() => {
    const desde = mesDesde(mesSel); const hasta = mesHasta(mesSel)
    return facturas.filter(f => { if (titularActivo && f.titular_id!==titularActivo.id) return false; if (!f.fecha_factura) return false; return f.fecha_factura>=desde && f.fecha_factura<=hasta })
  ...
  return (
    <div style={{ background: COLORS.bg, padding: '24px 28px', minHeight: '100%' }}>
      ...
      {bannerVisible && (
        <div style={{ background: '#e8f442', color: '#111111', padding: '6px 14px', fontSize: 12, fontFamily: 'Lexend, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, borderRadius: 8, marginBottom: 12 }}>
          <span>Plazo gestoría <strong style={{ fontWeight: 500 }}>{mesLabel(mesSel)}</strong>: hasta el {plazoStr}{diasPlazo > 0 ? ` · Quedan ${diasPlazo} día${diasPlazo===1?'':'s'}` : ' · ¡Hoy es el plazo!'}</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={() => setActiveTab('exportar')} style={{ padding: '4px 10px', fontSize: 11, fontFamily: 'Oswald, sans-serif', background: '#111111', color: '#e8f442', border: 'none', borderRadius: 5, cursor: 'pointer', fontWeight: 500, letterSpacing: '0.5px', textTransform: 'uppercase' }}>GENERAR ZIP</button>
            <button onClick={() => setBannerVisible(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#111111', padding: 2, display: 'flex', alignItems: 'center' }}><X size={14} /></button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{ color: COLORS.redSL, fontFamily: FONT.heading, fontSize: 22, fontWeight: 600, letterSpacing: '3px', margin: 0, textTransform: 'uppercase' }}>GESTOR DOCUMENTAL</h2>
          <span style={{ fontFamily: FONT.body, fontSize: 13, color: COLORS.mut, display: 'block', marginTop: 4 }}>{mesLabel(mesSel)}</span>
        </div>
      ...
      {activeTab === 'facturas' && (
        <>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 14, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            <input type="text" placeholder="Buscar proveedor, NIF, importe…" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} style={{ flex: 1, minWidth: 220, height: 36, padding: '0 12px', borderRadius: 8, border: `0.5px solid ${COLORS.brd}`, background: COLORS.card, fontSize: 13, fontFamily: FONT.body, color: COLORS.pri, outline: 'none' }} />
            <div style={{ ...islaStyle, padding: 0, overflow: 'hidden' }}>
              <select value={categoriaId} onChange={(e) => setCategoria(e.target.value)} style={{ ...DROPDOWN_BTN, border: 'none', background: 'transparent', minWidth: 280, height: 36, paddingRight: 28, cursor: 'pointer' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 14 }}>
            <div style={{ background: COLORS.card, border: `0.5px solid ${COLORS.brd}`, borderRadius: 14, padding: 14, fontSize: 13, fontFamily: FONT.body, alignSelf: 'start' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, paddingBottom: 8, borderBottom: `0.5px solid ${COLORS.brd}` }}>
                ...
                {driveFiltro.anio && <button type="button" onClick={() => setDriveFiltro({})} style={{ fontSize: 10, padding: '3px 9px', border: 'none', background: COLORS.group, borderRadius: 4, color: COLORS.sec, cursor: 'pointer', fontFamily: FONT.body }}>limpiar</button>}
              </div>
              {loading && <div style={{ color: COLORS.mut, fontSize: 12 }}>Cargando…</div>}
              {!loading && driveTree.map(tNode => (
                <NodoArbolItem key={tNode.label} node={tNode} level={0} filtroActivo={driveFiltro} expansionMap={expansionMap} titularColor={titularKey==='ruben' ? COLOR_RUBEN : COLOR_EMILIO} onSelect={setDriveFiltro} onToggleExpand={(key) => setExpansionMap(m => ({ ...m, [key]: !m[key] }))} />
              ))}
            </div>
                  ...
                  <thead>
                    <tr>
                      {HEADERS.map(h => {
                        ...
                        return <th key={h.col} onClick={() => handleSort(h.col)} style={{ fontFamily: FONT.heading, fontSize: 10, fontWeight: 500, letterSpacing: '2px', color: isActive ? COLORS.redSL : COLORS.mut, textTransform: 'uppercase', textAlign: h.align, padding: '10px 12px', background: COLORS.group, borderBottom: `0.5px solid ${COLORS.brd}`, whiteSpace: 'nowrap', cursor: 'pointer', userSelect: 'none' }}>{h.label}{isActive ? (sortDir==='asc' ? ' ↑' : ' ↓') : ''}</th>
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {!loading && facturasOrdenadas.map((f, idx) => {
                      ...
                      return (
                        <tr key={f.id} onClick={() => f.pdf_drive_url && window.open(f.pdf_drive_url, '_blank', 'noopener')} style={{ cursor: f.pdf_drive_url ? 'pointer' : 'default' }} onMouseEnter={(e) => (e.currentTarget.style.background = COLORS.bg)} onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
                          ...
                          <td style={{ ...tdStyle, fontSize: 12 }}>
                            {tit ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 6, fontFamily: FONT.body, fontSize: 12, fontWeight: 500, background: `${titColor}15`, color: titColor }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: titColor }} />{tit.nombre}</span> : <span style={{ color: COLORS.mut }}>—</span>}
                          </td>
                          {f.pdf_drive_url ? (
                            <td style={tdDoc} onClick={(e) => { e.stopPropagation(); window.open(f.pdf_drive_url!, '_blank', 'noopener,noreferrer') }} title="Ver factura"><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', minHeight: 38, fontSize: 22, lineHeight: 1, color: COLORS.pri, cursor: 'pointer', userSelect: 'none' }}>📎</div></td>
                          ) : (
                            <td style={tdDoc}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', minHeight: 38, fontSize: 18, lineHeight: 1, color: '#F26B1F', fontWeight: 600 }}>✕</div></td>
                          )}
                    ...
                    {!loading && facturasOrdenadas.length === 0 && <tr><td colSpan={8} style={{ ...tdStyle, textAlign: 'center', color: COLORS.mut, padding: '40px 12px' }}>Sin facturas para los filtros seleccionados</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
      ...
      {activeTab === 'ventas' && <TabVentas titularKey={titularKey} setTitularKey={setTitularKey} />}
      {activeTab === 'exportar' && <TabExportar titularKey={titularKey} setTitularKey={setTitularKey} mesSel={mesSel} facturasMes={facturasMesExportar} plazoStr={plazoStr} diasPlazo={diasPlazo} />}
    </div>
  )
}
...
function TabVentas({ titularKey, setTitularKey }: { titularKey: 'ruben'|'emilio'; setTitularKey: (k:'ruben'|'emilio') => void }) {
  return (
    <>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 14, marginBottom: 14 }}><ToggleTitular titularKey={titularKey} onChange={setTitularKey} /></div>
      <div style={{ marginTop: 24, padding: 60, textAlign: 'center', background: COLORS.card, border: `0.5px solid ${COLORS.brd}`, borderRadius: 14, color: COLORS.mut, fontFamily: FONT.body, fontSize: 14 }}>Subida de resúmenes de ventas (Uber Eats CSV) · Próximamente</div>
    </>
  )
}

function TabExportar({ titularKey, setTitularKey, mesSel, facturasMes, plazoStr, diasPlazo }: { titularKey: 'ruben'|'emilio'; setTitularKey: (k:'ruben'|'emilio') => void; mesSel: MesSel; facturasMes: FacturaRow[]; plazoStr: string; diasPlazo: number }) {
    ...
    <>
      ...
      <div style={{ background: COLORS.card, border: `0.5px solid ${COLORS.brd}`, borderRadius: 14, padding: '20px 22px', marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 12, color: todoOk ? '#3B6D11' : '#BA7517', fontWeight: 500 }}>{[facturasConfirmadas, ventasUberSubido].filter(Boolean).length} de 2 listos</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: facturasConfirmadas ? '#EAF3DE' : '#FCEBEB', borderRadius: 8 }}>
            <span style={{ display: 'flex', width: 22, height: 22, borderRadius: '50%', background: facturasConfirmadas ? '#639922' : '#A32D2D', color: '#fff', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>{facturasConfirmadas ? '✓' : '✕'}</span>
            <span style={{ flex: 1, fontSize: 13, color: facturasConfirmadas ? '#173404' : '#501313', fontWeight: 500 }}>Todas las facturas del mes importadas</span>
            {facturasConfirmadas ? (
              <span style={{ fontSize: 12, color: '#3B6D11' }}>{numFacturas} facturas</span>
            ) : (
              <button onClick={() => setFacturasConfirmadas(true)} style={{ padding: '4px 12px', borderRadius: 6, border: 'none', background: '#A32D2D', color: '#fff', fontSize: 12, fontFamily: FONT.body, cursor: 'pointer', fontWeight: 500 }}>Confirmar</button>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: ventasUberSubido ? '#EAF3DE' : '#FCEBEB', borderRadius: 8 }}>
            <span style={{ display: 'flex', width: 22, height: 22, borderRadius: '50%', background: ventasUberSubido ? '#639922' : '#A32D2D', color: '#fff', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>{ventasUberSubido ? '✓' : '✕'}</span>
            <span style={{ flex: 1, fontSize: 13, color: ventasUberSubido ? '#173404' : '#501313', fontWeight: 500 }}>Ventas Uber Eats subidas</span>
            <span style={{ fontSize: 12, color: ventasUberSubido ? '#3B6D11' : '#A32D2D' }}>{ventasUberSubido ? 'Subido' : 'Pendiente'}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: COLORS.card, border: `0.5px solid ${COLORS.brd}`, borderRadius: 14, padding: '20px 22px', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13, fontFamily: FONT.body }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={{ fontSize: 16 }}>📁</span><span><strong style={{ fontWeight: 500 }}>Ventas</strong> · {numResumenesUber} Resumen{numResumenesUber===1?'':'es'} Uber</span></div>
        </div>
      </div>

      <button disabled={!todoOk} style={{ width: '100%', padding: '14px 20px', background: todoOk ? '#111' : '#d0c8bc', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, fontFamily: FONT.body, cursor: todoOk ? 'pointer' : 'not-allowed' }}>
...
interface NodoArbolItemProps { node: DriveNode; level: number; filtroActivo: DriveFiltro; expansionMap: Record<string,boolean>; titularColor: string; onSelect: (f:DriveFiltro) => void; onToggleExpand: (k:string) => void }
...
function NodoArbolItem({ node, level, filtroActivo, expansionMap, titularColor, onSelect, onToggleExpand }: NodoArbolItemProps) {
  ...
  // tipos explícitos string/number para evitar error TS literal estricto
  ...
  const rowStyle: CSSProperties = { width: '100%', display: 'flex', alignItems: 'center', padding: '6px 8px', paddingLeft: 6 + level*12, background: nodoBg, border: 'none', borderLeft: nodoBL, borderRadius: node.kind==='mes' ? '0 4px 4px 0' : '0 6px 6px 0', cursor: 'pointer', fontFamily: nodoFF, fontSize: nodoFS, textAlign: 'left', color: nodoColor, fontWeight: nodoFW, opacity: sinFacturas && !esActivo ? 0.5 : 1, marginBottom: node.kind==='titular' ? 4 : 1, letterSpacing: node.kind==='titular' ? '1px' : 'normal', textTransform: node.kind==='titular' ? 'uppercase' : 'none' }

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 1 }}>
        {tieneHijos ? (
          <button type="button" onClick={(e) => { e.stopPropagation(); onToggleExpand(myKey) }} style={{ width: 24, height: 28, padding: 0, border: 'none', background: 'transparent', cursor: 'pointer', color: COLORS.sec, fontSize: 18, fontWeight: 700, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>
            {expandido ? '▾' : '▸'}
          </button>
      ...
      {expandido && tieneHijos && (
        <div>{node.children!.map((child, idx) => <NodoArbolItem key={`${child.label}-${idx}`} node={child} level={level+1} filtroActivo={filtroActivo} expansionMap={expansionMap} titularColor={titularColor} onSelect={onSelect} onToggleExpand={onToggleExpand} />)}</div>
      )}
    </div>
  )
}
