...
function TabCalendario() {
  ...
  const hoy30 = addDays(new Date(), 30)
  const items30 = items.filter(i => i.fecha <= hoy30)
  const totalCobros = items30.filter(i => i.tipo === 'COBRO').reduce((s, i) => s + i.importe, 0)
  const totalPagos = items30.filter(i => i.tipo === 'PAGO').reduce((s, i) => s + i.importe, 0)
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
        <KpiCard label="Cobros próximos 30 días" value={fmtEur(totalCobros)} color="#1D9E75" />
        <KpiCard label="Pagos próximos 30 días" value={fmtEur(totalPagos)} color="#B01D23" />
        <KpiCard label="Saldo neto estimado" value={fmtEur(balance)} color={balance >= 0 ? '#1D9E75' : '#B01D23'} />
      </div>
...
function TabGastos() {
  ...
  async function eliminar(id: number) {
    ...
    if (error) showToast('Error al eliminar: ' + error.message, false)
    else { showToast('Gasto eliminado', true); cargar() }
  }
    ...
    <div>
      ...
      {showForm && (
        <div style={{ backgroundColor: '#1a1a1a', border: '0.5px solid #2a2a2a', borderRadius: 12, padding: '20px 24px', marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', gap: 12, alignItems: 'end' }}>
            ...
            <InputField label="Categoría" value={form.categoria} onChange={v => setForm(f => ({ ...f, categoria: v }))} />
          </div>
      ...
      <div style={{ background: '#141414', border: '0.5px solid #2a2a2a', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#0a0a0a' }}>
              {['Concepto', 'Importe', 'Periodicidad', 'Próximo pago', 'Categoría', 'Estado', 'Acciones'].map(h => (
                <th key={h} style={{ fontFamily: 'Oswald, sans-serif', fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase', color: '#777777', padding: '12px 16px', textAlign: 'left', fontWeight: 500 }}>
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {gastos.length === 0 && (
              <tr>
                <td colSpan={7} style={{ padding: 32, textAlign: 'center', color: '#777777', fontSize: 13 }}>
            ...
            {gastos.map(g => (
              ...
              >
                ...
                <td style={{ padding: '12px 16px', fontSize: 12, color: '#777777' }}>{g.categoria ?? '—'}</td>
                <td style={{ padding: '12px 16px' }}>
                ...
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button
                      ...
                      style={{ fontSize: 12, fontFamily: 'Oswald, sans-serif', padding: '5px 12px', borderRadius: 6, border: 'none', backgroundColor: '#B01D23', color: '#ffffff', cursor: 'pointer' }}
                    >
                      ELIMINAR
                    </button>
                  </div>
...
function TabHistorial() {
  ...
  if (noTable) return (
    <div style={{ padding: '24px', borderRadius: 8, backgroundColor: '#1a1a1a', border: '0.5px solid #2a2a2a', color: '#777777', fontSize: 13 }}>
      La tabla <code style={{ color: '#e8f442' }}>conciliacion</code> no existe aún. El historial estará disponible cuando se active el módulo de conciliación bancaria.
    </div>
  )
