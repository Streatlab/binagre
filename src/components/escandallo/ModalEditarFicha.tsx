          ...
          {esReceta && (
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 96, height: 96, borderRadius: 0, border: '1px solid var(--sl-border)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--sl-input-edit)', flexShrink: 0 }}>
                  {fotoUrl
                    ? <img src={fotoUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : <ImagePlus size={26} color="var(--sl-text-muted)" />}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'transparent', border: '1px dashed var(--sl-border)', borderRadius: 0, color: 'var(--sl-text-secondary)', padding: '7px 14px', fontSize: 13, cursor: 'pointer', width: 'fit-content' }}>
            ...
            <div className="space-y-2">
              {ingredientes.map((i, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  ...
                  <button onClick={() => delIng(idx)} style={{ gridColumn: 'span 1', background: 'transparent', border: '0.5px solid var(--sl-border)', borderRadius: 0, color: GRANATE, cursor: 'pointer', padding: 7, display: 'flex', justifyContent: 'center' }}><Trash2 size={14} /></button>
                </div>
              ))}
              <button onClick={addIng} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: '1px dashed var(--sl-border)', borderRadius: 0, color: 'var(--sl-text-secondary)', padding: '7px 14px', fontSize: 13, cursor: 'pointer' }}>
            ...
            <div className="space-y-2">
              {pasos.map((p, idx) => (
                <div key={idx} className="flex gap-2 items-start">
                  ...
                  <button onClick={() => delPaso(idx)} style={{ background: 'transparent', border: '0.5px solid var(--sl-border)', borderRadius: 0, color: GRANATE, cursor: 'pointer', padding: 7, marginTop: 1 }}><Trash2 size={14} /></button>
                </div>
              ))}
              <button onClick={addPaso} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: '1px dashed var(--sl-border)', borderRadius: 0, color: 'var(--sl-text-secondary)', padding: '7px 14px', fontSize: 13, cursor: 'pointer' }}>
              ...
              {ALERGENOS_14.map(a => {
                ...
                return (
                  <button key={a} type="button" onClick={() => toggleAlerg(a)}
                    style={{ padding: '7px 14px', borderRadius: 0, fontFamily: 'Lexend, sans-serif', fontSize: 13, cursor: 'pointer', border: on ? 'none' : '1px solid var(--sl-border)', background: on ? GRANATE : 'transparent', color: on ? BLANCO : 'var(--sl-text-secondary)' }}>
                    {a}
                  </button>
                )
              })}
