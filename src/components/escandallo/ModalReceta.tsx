...
export default function ModalReceta({ receta, ingredientes, epsList, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-card border border-[#333355] rounded-xl w-full max-w-7xl my-8 shadow-2xl" onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-6 gap-3">
            <div className="sm:col-span-2">
              <label className="block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider">Nombre</label>
              <input className={inputCls} value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej: Smash Burger" />
            </div>
            <div>
              <label className="block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider">Categoría</label>
            ...
            <div>
              <label className="block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider">Raciones</label>
              <input type="number" min={1} step="1" className={inputCls} value={raciones || ''} onChange={e => setRaciones(parseFloat(e.target.value) || 1)} />
            </div>
            <div>
              <label className="block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider">Tamaño rac</label>
              <input type="number" min={0} step="any" className={inputCls} value={tamanoRac || ''} onChange={e => setTamanoRac(parseFloat(e.target.value) || 0)} />
            </div>
            <div>
              <label className="block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider">Unidad</label>
              <select className={inputCls} value={unidad} onChange={e => setUnidad(e.target.value)}>
                {UNIDADES.map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </div>
            <div className="sm:col-span-1">
              <label className="block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider">Fecha</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] text-[#aaa] uppercase tracking-wider">Líneas</p>
              ...
              <div className="border border-[#333355] rounded-lg overflow-hidden">
                    ...
                    <tbody>
                      ...
                      {lineasCalc.map((l, idx) => (
                        <tr key={idx}>
                          ...
                          <td className={tdCls}>
                            <input list={`r-i-${idx}`} className="w-full bg-transparent border-none outline-none text-sm text-white placeholder:text-[#666]" value={l.ingrediente_nombre} onChange={e => selectItem(idx, e.target.value)} placeholder={l.tipo === 'ING' ? 'Ingrediente...' : 'EPS...'} />
                ...
                <div className="flex items-center justify-between px-3 py-3 border-t-2 border-accent/30 bg-accent/5">
                  <div className="flex items-center gap-6">
                    <div><span className="text-[10px] text-[#888] uppercase tracking-wide block">Coste tanda</span><span className="text-sm font-bold text-white">{costeTanda.toFixed(4)} €</span></div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
              {waterfallCanales.map(({ canal, pvpKey, pvp, w }) => {
                return (
                  <div key={canal.id || canal.canal} className="bg-[#1e1e2e] border border-[#333355] rounded-lg overflow-hidden">
                    <div className="px-3 py-2 bg-[#1e1e2e] border-b border-[#333355]">
                    ...
                    <div className="grid grid-cols-2 text-[11px]">
                      <div className="bg-[#1e3a1e]/40 p-2 border-r border-[#333355]">
                    ...
                    <div className="grid grid-cols-2 border-t border-[#333355]">
                    ...
                    <div className="grid grid-cols-2 border-t border-[#333355]">
                      <div className="bg-[#1e3a1e]/30 p-2 text-[10px] text-white flex justify-between col-span-2">
                        ...
                        <input type="number" min={0} step="0.01" value={pvp || ''}
                          ...
                          className="w-20 bg-[#1e1e2e] border border-[#555] rounded px-1 text-right text-white text-xs" />
                      </div>
                    </div>
