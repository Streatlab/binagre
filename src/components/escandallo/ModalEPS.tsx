...
export default function ModalEPS({ eps, ingredientes, onClose, onSaved, onDelete }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-[var(--sl-card)] border border-[var(--sl-border)] rounded-xl w-full max-w-5xl my-8 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-[1fr_80px_80px_100px_130px] gap-2 items-end">
            ...
            <div>
              <label className="block text-[11px] text-[var(--sl-text-muted)] mb-1 uppercase tracking-wider">Raciones</label>
              <input type="number" min={1} step="1" className={inputCls} value={raciones || ''} onChange={e => setRaciones(parseFloat(e.target.value) || 1)} />
            </div>
            <div>
              <label className="block text-[11px] text-[var(--sl-text-muted)] mb-1 uppercase tracking-wider">Tamaño rac</label>
              <input type="number" min={0} step="any" className={inputCls} value={tamanoRac || ''} onChange={e => setTamanoRac(parseFloat(e.target.value) || 0)} />
            </div>
            <div>
              <label className="block text-[11px] text-[var(--sl-text-muted)] mb-1 uppercase tracking-wider">Unidad</label>
                    ...
                    <tbody>
                      ...
                      {lineasCalc.map((l, idx) => {
                        ...
                        <tr key={idx}>
                          ...
                          <td className={tdCls + ' text-right'}><input type="number" min={0} step="0.000001" className="w-full bg-transparent border-none outline-none text-sm text-[var(--sl-text-primary)] text-right" value={l.eur_ud_neta ? Number(l.eur_ud_neta).toFixed(2) : ''} onChange={e => updateLinea(idx, { eur_ud_neta: parseFloat(e.target.value) || 0 })} /></td>
