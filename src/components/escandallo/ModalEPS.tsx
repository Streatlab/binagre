...
export default function ModalEPS({ eps, ingredientes, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl w-full max-w-5xl my-8 shadow-2xl" onClick={e => e.stopPropagation()}>
        {/* Header: ID + Nombre + EP + Fecha */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#2a2a2a]">
          <div>
            <h3 className="text-base font-semibold text-[#ffffff]">{eps ? 'Editar EPS' : 'Nueva EPS'}</h3>
            {eps?.codigo && <p className="text-xs text-[#999999] mt-0.5 font-mono">{eps.codigo} · EPS</p>}
          </div>
          <button onClick={onClose} className="text-[#999999] hover:text-[#ffffff] transition text-xl leading-none">×</button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            <div className="col-span-2">
              <label className="block text-[11px] text-[#999999] mb-1 uppercase tracking-wider">Nombre</label>
              <input className={inputCls} value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej: Salsa brava" />
            </div>
            <div>
              <label className="block text-[11px] text-[#999999] mb-1 uppercase tracking-wider">Raciones</label>
              <input type="number" min={1} step="1" className={inputCls} value={raciones || ''} onChange={e => setRaciones(parseFloat(e.target.value) || 1)} />
            </div>
            <div>
              <label className="block text-[11px] text-[#999999] mb-1 uppercase tracking-wider">Tamaño rac</label>
              <input type="number" min={0} step="any" className={inputCls} value={tamanoRac || ''} onChange={e => setTamanoRac(parseFloat(e.target.value) || 0)} />
            </div>
            <div>
              <label className="block text-[11px] text-[#999999] mb-1 uppercase tracking-wider">Unidad</label>
              <select className={inputCls} value={unidad} onChange={e => setUnidad(e.target.value)}>
                {UNIDADES.map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </div>
            <div className="col-span-2 sm:col-span-1">
              <label className="block text-[11px] text-[#999999] mb-1 uppercase tracking-wider">Fecha</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] text-[#999999] uppercase tracking-wider">Líneas</p>
              ...
              <div className="border border-[#2a2a2a] rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full" style={{ minWidth: '800px' }}>
                    ...
                    <tbody>
                      {!lineasCalc.length && <tr><td colSpan={8} className="px-3 py-6 text-center text-[#999999] text-sm">Sin líneas — añade ingredientes</td></tr>}
                      {lineasCalc.map((l, idx) => (
                        <tr key={idx}>
                          <td className={tdCls + ' text-[#999999]'}>{idx + 1}</td>
                          <td className={tdCls}>
                            <input list={`eps-ing-${idx}`} className="w-full bg-transparent border-none outline-none text-sm text-[#ffffff] placeholder:text-[#cccccc]" value={l.ingrediente_nombre} onChange={e => selectIngrediente(idx, e.target.value)} placeholder="Buscar ingrediente…" />
                            <datalist id={`eps-ing-${idx}`}>{ingredientes.map(i => <option key={i.id} value={i.nombre} />)}</datalist>
                          </td>
                          <td className={tdCls + ' text-right'}><input type="number" min={0} step="any" className="w-full bg-transparent border-none outline-none text-sm text-[#ffffff] text-right" value={l.cantidad || ''} onChange={e => updateLinea(idx, { cantidad: parseFloat(e.target.value) || 0 })} /></td>
                          <td className={tdCls}><select className="w-full bg-transparent border-none outline-none text-sm text-[#ffffff]" value={l.unidad} onChange={e => updateLinea(idx, { unidad: e.target.value })}>{UNIDADES.map(u => <option key={u} value={u}>{u}</option>)}</select></td>
                          <td className={tdCls + ' text-right'}><input type="number" min={0} step="0.000001" className="w-full bg-transparent border-none outline-none text-sm text-[#ffffff] text-right" value={l.eur_ud_neta || ''} onChange={e => updateLinea(idx, { eur_ud_neta: parseFloat(e.target.value) || 0 })} /></td>
                          <td className={tdCls + ' text-right font-medium text-[#ffffff]'}>{fmtES(l.eur_total, 4)}</td>
                          <td className={tdCls + ' text-right text-[#999999]'}>{fmtES(l.pct_total, 1)}%</td>
                          <td className={tdCls}><button onClick={() => deleteLinea(idx)} className="text-[#999999] hover:text-[#dc2626] transition text-sm">×</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex items-center justify-between px-3 py-3 border-t-2 border-accent/40 bg-accent/5">
                  <div className="flex items-center gap-6">
                    <div>
                      <span className="text-[10px] text-[#999999] uppercase tracking-wide block">Coste tanda</span>
                      <span className="text-sm font-bold text-[#ffffff]">{fmtEurES(costeTanda, 4)}</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-[#999999] uppercase tracking-wide block">Coste ración</span>
                      <span className="text-base font-bold text-[#ffffff]">{fmtEurES(costeRac, 4)}</span>
                    </div>
                  </div>
