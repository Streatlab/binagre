...
export default function TabMermas({ mermas, onSelect, onNew }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!mermas.length ? (
        <div className="bg-card border border-[#444] rounded-xl p-12 text-center">
          ...
          <div className="overflow-x-auto max-h-[calc(100vh-240px)] overflow-y-auto">
            <table className="w-full" style={{ minWidth: '2400px' }}>
              <thead className="sticky top-0 z-20">
                <tr>
                  <th className={thCls + ' sticky left-0 z-30 bg-[#2a2a2a]'} style={{ minWidth: 90 }}>IDING</th>
                  <th className={thCls}>CATEGORIA</th>
                  <th className={thCls + ' sticky z-30 bg-[#2a2a2a]'} style={{ left: 90, minWidth: 220 }}>NOMBRE BASE</th>
              ...
              <tbody>
                {mermas.map(m => (
                  <tr key={m.id} onClick={() => onSelect?.(m)} className="cursor-pointer hover:bg-[#383838] transition-colors">
