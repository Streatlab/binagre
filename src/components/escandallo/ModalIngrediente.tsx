...
const inputCls = 'w-full bg-[#2a2a2a] border border-[#444] rounded-lg px-3 py-2 text-sm text-[#f0f0f0] placeholder:text-[#666] focus:outline-none focus:border-accent'
const labelCls = 'block text-[11px] text-[#aaa] mb-1 uppercase tracking-wider'
const btnPrimary = 'px-4 py-2 bg-accent text-black text-sm font-semibold rounded-lg hover:brightness-110 transition'
const btnSecondary = 'px-4 py-2 text-sm text-[#aaa] border border-[#444] rounded-lg hover:text-white hover:border-[#666] transition'
...
export default function ModalIngrediente({ ingrediente, onClose, onSaved, onOpenMerma }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-[#333] border border-[#444] rounded-xl w-full max-w-5xl my-8 shadow-2xl" onClick={e => e.stopPropagation()}>
