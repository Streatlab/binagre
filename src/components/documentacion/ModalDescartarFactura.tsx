...
const inp: React.CSSProperties = {
  ...
  borderRadius: 0, color: BLANCO, fontFamily: 'Lexend, sans-serif', fontSize: 13, padding: '10px 12px', outline: 'none',
}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: INK, borderRadius: 0, width: '100%', maxWidth: 460, border: `1px solid ${GRIS}`, padding: 22 }}
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 14 }}>
          ...
          {modo === 'siempre' && (
            <div style={{ marginLeft: 24, display: 'flex', flexDirection: 'column', gap: 8, background: INK, border: `1px solid ${GRANATE}`, borderRadius: 0, padding: '10px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} disabled={guardando}
            style={{ padding: '9px 16px', background: INK, border: `1px solid ${GRIS}`, borderRadius: 0, color: BLANCO, fontFamily: 'Oswald, sans-serif', fontSize: 12, letterSpacing: 1, textTransform: 'uppercase', cursor: guardando ? 'default' : 'pointer' }}>
            Cancelar
          </button>
          <button onClick={confirmar} disabled={guardando || bloqueadoSiempre}
            style={{ padding: '9px 16px', background: GRANATE, border: 'none', borderRadius: 0, color: BLANCO, fontFamily: 'Oswald, sans-serif', fontSize: 12, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', cursor: (guardando || bloqueadoSiempre) ? 'not-allowed' : 'pointer', opacity: (guardando || bloqueadoSiempre) ? 0.5 : 1 }}>
