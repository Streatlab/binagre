/**
 * ResumenLanding v4 — pestaña Resumen del Panel Global.
 * Brutalismo + alternancia de fondos (ninguna sección contigua repite fondo).
 * Formato numérico ÚNICO del ERP (@/lib/format): separadores de miles SIEMPRE,
 * € reducido al mínimo (solo facturación y neto del hero). "TM" siempre en
 * mayúscula. Barra de canales con track de color propio (progreso legible).
 * Textos de la card oscura con más contraste. Ranking de marcas (campo servicio).
 * Bloques que navegan a otras pestañas. No calcula nada: todo llega por props.
 */
...
const CREMA = '#FCEFD6'
const CLARO = '#F3D9A8'
const TRACK = '#ecdcb8'        // fondo de las barras de progreso (contraste con card)
const ROSA = '#FF2E63'
const AMA = '#FFC400'
const VERDE = '#0FB86B'
const NAR = '#FF6A1A'
...
// textos sobre fondo oscuro (más contraste que v3)
const D1 = CREMA
const D2 = '#e3d9c2'
const D3 = '#cabd9f'
...
const EUR = (n: number) => fmtEur(n, { decimals: 0 })                       // con €  (solo hero/neto)
const E = (n: number) => fmtEur(n, { showEuro: false, decimals: 0 })        // sin €, entero
const E2 = (n: number) => fmtEur(n, { showEuro: false, decimals: 2 })       // sin €, 2 dec
const N = (n: number) => fmtNum(n, 0)                                       // entero con miles
...
/* barra de progreso brutalista: nombre fijo + track con color propio + relleno + % */
function Barra({ nombre, pct, color, valor }: { nombre: string; pct: number; color: string; valor: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <span style={{ ...d('15px'), width: 96, flexShrink: 0 }}>{nombre}</span>
      <div style={{ position: 'relative', flex: 1, height: 30, background: TRACK, border: `3px solid ${INK}`, overflow: 'hidden' }}>
        <div style={{ width: `${Math.min(100, Math.max(0, pct))}%`, height: '100%', background: color }} />
        <span style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', ...d('16px') }}>{P0(pct)}</span>
      </div>
      <span style={{ ...d('16px'), width: 92, textAlign: 'right', flexShrink: 0 }}>{valor}</span>
    </div>
  )
}
...
export default function ResumenLanding(p: Props) {
  ...
  const pnl: Array<[string, string, string, boolean]> = [
    ['Facturación', E2(p.ventasPeriodo), D1, false],
    ['Ingresos netos', E2(p.netoEstimado), D1, false],
    ['Producto · COGS', p.grupos.producto.gasto > 0 ? '−' + E2(p.grupos.producto.gasto) : DI, p.grupos.producto.gasto > 0 ? NAR_CL : D3, false],
    ['Margen bruto', E2(margenBruto), AMA, true],
    ['Equipo · Labor', p.grupos.equipo.gasto > 0 ? '−' + E2(p.grupos.equipo.gasto) : DI, p.grupos.equipo.gasto > 0 ? NAR_CL : D3, false],
    ['Local · Occupancy', p.grupos.local.gasto > 0 ? '−' + E2(p.grupos.local.gasto) : DI, p.grupos.local.gasto > 0 ? NAR_CL : D3, false],
    ['Controlables · Opex', p.grupos.controlables.gasto > 0 ? '−' + E2(p.grupos.controlables.gasto) : DI, p.grupos.controlables.gasto > 0 ? NAR_CL : D3, false],
    ['Resultado neto', E2(resultadoNetoPL), resultadoNetoPL >= 0 ? VERDE_CL : '#ff9aa8', true],
  ]
  const desv = [
    ...
    { v: DELTA(p.variacionTM), l: 'TM vs media', c: p.variacionTM != null && p.variacionTM < 0 ? ROSA : VERDE, big: true },
    { v: E(p.ebitda), l: 'resultado', c: p.ebitda >= 0 ? VERDE : ROSA, big: false },
  ]
    ...
    <div style={{ background: CREMA, fontFamily: LEX, color: INK, border: `4px solid ${INK}`, marginTop: 4 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1.45fr 1fr', borderBottom: `4px solid ${INK}`, background: AMA }}>
        <div style={{ padding: `42px ${PAD} 40px`, borderRight: `4px solid ${INK}` }}>
          ...
          <div style={{ ...d('clamp(32px,4.2vw,56px)'), margin: '16px 0 16px', maxWidth: 600 }}>{p.pedidosPeriodo > 0 ? `Has entregado ${N(p.pedidosPeriodo)} pedidos.` : 'Aún no hay pedidos entregados.'}</div>
        ...
        <div style={{ background: CLARO, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 22 }}>
          <div style={{ background: '#fff', border: `3px solid ${INK}`, boxShadow: `10px 10px 0 ${INK}`, padding: '26px 28px', width: '100%', maxWidth: 380 }}>
            ...
            {[['TM bruto', E2(p.tmBruto)], ['TM neto', E2(p.tmNeto)], ['EBITDA', E(p.ebitda)], ['Margen', P0(p.ebitdaPct)]].map(([l, v], i) => (
      ...
      {/* 5 · CANALES (blanco) — barras con track propio */}
      <section style={sec('#fff')}>
        <Title tag="Por dónde entra el hambre" tagBg={AMA} title="Reparto por canal: peso, bruto, neto, margen, pedidos y € por pedido." nav={{ label: 'Operaciones', onClick: () => p.onNavTab?.('operaciones') }} />
          ...
          {p.canalStats.map(c => (
            <div key={c.id} style={{ border: `3px solid ${INK}`, background: CREMA, boxShadow: `5px 5px 0 ${INK}`, padding: '14px 16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6 }}>
                {[['Neto', E2(c.neto), VERDE], ['Margen', P2(c.margen), INK], ['Pedidos', N(c.pedidos), INK], ['TM', c.pedidos > 0 ? E2(c.ticket) : '—', AZUL], ['€/ped neto', c.pedidos > 0 ? E2(c.neto / c.pedidos) : '—', VERDE]].map(([l, v, col]) => (
                  <div key={l as string}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 22 }}>
          <div style={{ border: `2px solid #ffffff33` }}>
            {pnl.map(([l, v, c, bold], i) => (
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '13px 16px', borderTop: i > 0 ? `1px solid #ffffff1f` : 'none', background: bold ? '#ffffff12' : 'transparent' }}>
                <span style={{ fontSize: 15, fontWeight: bold ? 700 : 500, color: bold ? D1 : D2 }}>{l}</span>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {grupos.map(g => {
                ...
                return (
                  <div key={g.k} style={{ border: `2px solid #ffffff33`, padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                      <span style={{ fontFamily: OSW, fontSize: 16, letterSpacing: '0.5px', textTransform: 'uppercase', color: D1 }}>{g.lbl} <span style={{ fontSize: 12, color: D3 }}>{g.sub}</span></span>
                      <span style={d('clamp(20px,2.4vw,26px)', D1)}>{gd.gasto > 0 ? E(gd.gasto) : DI}</span>
                    </div>
                    <div style={{ fontFamily: LEX, fontSize: 14, fontWeight: 600, color: D2, marginTop: 6 }}>{Math.round(gd.pctSobreNetos)}% s/ neto · objetivo {g.obj}% · pres. <span style={{ color: AMA, fontFamily: OSW, fontSize: 18 }}><Edit value={gd.presupuesto} onSave={v => p.onSavePresupuestoGrupo(g.k, v)} color={AMA} /></span></div>
                  </div>
                )
              })}
            </div>
            ...
            return (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                  <span style={d('22px')}>Hoy · {N(p.diario!.real)} <span style={d('22px', colorSemaforo(prog))}>{P0(prog)}</span></span>
                  <span style={d('20px')}>Faltan <span style={{ color: colorSemaforo(prog) }}>{N(faltan)}</span> de objetivo <span style={d('20px')}>{N(p.diario!.objetivo)}</span></span>
                </div>
                <div style={{ height: 18, border: `3px solid ${INK}`, background: '#fff' }}><div style={{ width: `${Math.min(100, prog)}%`, height: '100%', background: colorSemaforo(prog) }} /></div>
              </div>
            )
          })()}
          {objetivos.map(o => {
            ...
            return (
              <div key={o.k}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                  <span style={d('22px')}>{o.lbl} · {N(o.real)} <span style={d('22px', colorSemaforo(prog))}>{P0(prog)}</span></span>
                  <span style={d('20px')}>Faltan <span style={{ color: colorSemaforo(prog) }}>{N(faltan)}</span> de objetivo <Edit value={o.obj} onSave={v => p.onSaveObjetivoVenta(o.k, v)} /></span>
                </div>
                <div style={{ height: 16, border: `3px solid ${INK}`, background: '#fff' }}><div style={{ width: `${Math.min(100, prog)}%`, height: '100%', background: colorSemaforo(prog) }} /></div>
              </div>
            )
          })}
        </div>
      ...
      {/* 8 · DÍAS PICO (CREMA) + PROYECCIONES (VERDE) */}
      ...
      {/* 10 · TOP (blanco) + PROVISIONES (CLARO) */}
      ...
      {/* 11 · MARCAS (AMA) */}
      <section style={sec(AMA)}>
        <Title tag="Tus marcas" tagBg={ROSA} tagColor="#fff" title="Qué marca tira más este periodo." nav={{ label: 'Marcas', onClick: () => p.onNavTab?.('marcas') }} />
        {p.marcasServicio && p.marcas.length > 0
          ? <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {p.marcas.slice(0, 6).map(mk => (
                <div key={mk.nombre} style={{ border: `3px solid ${INK}`, background: '#fff', boxShadow: `5px 5px 0 ${INK}`, padding: '12px 16px' }}>
                  <Barra nombre={mk.nombre} pct={mk.pct} color={ROSA} valor={E2(mk.bruto)} />
                  <div style={{ fontFamily: LEX, fontSize: 13, fontWeight: 600, marginTop: 8, opacity: 0.8 }}>Neto {E2(mk.neto)} · {N(mk.pedidos)} pedidos · TM {mk.pedidos > 0 ? E2(mk.bruto / mk.pedidos) : '—'}</div>
                </div>
              ))}
            </div>
          : <div style={{ border: `3px solid ${INK}`, background: '#fff', padding: '20px 18px', fontFamily: LEX, fontWeight: 600 }}>Sin datos de marca en este periodo (el campo «servicio» no viene informado en las ventas cargadas). Abre la pestaña Marcas para el detalle y la activación.</div>}
      </section>
