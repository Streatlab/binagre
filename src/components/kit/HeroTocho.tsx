...
export default function HeroTocho(props: {
  ...
  return (
    <div style={{ background: AMA, border: BORDER, boxShadow: SHADOW, padding: 16 }}>
      <span style={{ background: '#fff', border: BORDER_FINO, padding: '2px 10px', fontFamily: OSW, fontSize: 11, letterSpacing: '0.1em' }}>{claim}</span>
      ...
      {(dato || delta) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          {delta && <span style={{ background: deltaPositivo ? VERDE : '#B01D23', color: '#fff', border: BORDER_FINO, padding: '3px 9px', fontFamily: OSW, fontSize: 14, fontWeight: 700 }}>{delta}</span>}
        </div>
      )}
      {netoTexto && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: VERDE, border: BORDER, boxShadow: SHADOW_MINI, padding: '7px 13px', marginTop: 10, color: '#fff', fontFamily: OSW, fontWeight: 700 }}>
          {netoTexto}
        </div>
      )}
      {extra}
