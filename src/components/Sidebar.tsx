...
export default function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // pinned: abierto tras interactuar (con auto-cierre por inactividad)
  // peek: abierto temporal al pasar el ratón
  // cerrado: el usuario lo plegó a mano con « → manda sobre todo lo demás
  ...
  // Plegado a mano manda; si no, abierto mientras esté fijado o el ratón encima.
      ...
      >
        {collapsed ? (
          ...
          <div style={{ padding: 12, borderBottom: `1px solid ${T.brd}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 64 }}>
            ...
            <button onClick={(e) => { e.stopPropagation(); colapsarManual() }} style={{ color: T.mut, background: 'none', border: 'none', cursor: 'pointer', padding: 6, flexShrink: 0, minWidth: 44, minHeight: 44, display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="hover:text-[var(--sl-text-primary)] transition-colors hidden md:flex" title="Colapsar">«</button>
          </div>
        )}
