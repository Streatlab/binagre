# Job: REESCRIBIR src/pages/Ocr.tsx completo — el bot anterior no lo tocó

## Contexto crítico
El bot Claude Code anterior NO modificó `src/pages/Ocr.tsx` (sigue con SHA fd01275e). Necesito que ESTA EJECUCIÓN SÍ reescriba el archivo `src/pages/Ocr.tsx` completo, sin excusas. Sin esto, la página `/ocr` sigue mostrando "Error cargando facturas" porque consulta columnas inexistentes.

## INSTRUCCIÓN DIRECTA AL BOT

Reemplaza el contenido COMPLETO del archivo `src/pages/Ocr.tsx` con el contenido exacto que aparece más abajo entre los marcadores `--- INICIO ARCHIVO ---` y `--- FIN ARCHIVO ---`. No edites parcialmente. Sustituye el archivo entero. Esto NO toca ningún otro archivo del repo.

## Schema real de la tabla `facturas` en Supabase (VERIFICADO)
- id, fecha_factura, proveedor_nombre, total, tipo, estado
- pdf_drive_url, pdf_filename, titular_id, categoria_factura
- nif_emisor, numero_factura, ocr_confianza
- NO existe `fecha`, NO existe `importe`, NO existe `concepto`, NO existe `contraparte`, NO existe `categoria_id`, NO existe `conciliado`

Tabla N:N: `facturas_gastos` (id, factura_id, conciliacion_id, confirmado, importe_asociado).

--- INICIO ARCHIVO src/pages/Ocr.tsx ---
```tsx
...
## Verificación
1. Ejecutar `npm run build` antes de pushear. Si falla, arreglar antes.
2. Confirmar que el archivo `src/pages/Ocr.tsx` ya NO contiene las strings: `m.fecha`, `m.importe`, `m.concepto`, `m.contraparte`, `m.categoria_id`, `m.conciliado`, `r.importe`, `r.conciliado`, `eq('conciliado'`, `is.gte('fecha'`, `lte('fecha'`. Solo deben aparecer `fecha_factura`, `total`, `proveedor_nombre`, `nif_emisor`, `categoria_factura`, `facturas_gastos`.

## Commit
```
fix(ocr): reescribe Ocr.tsx completo con schema real de facturas (fecha_factura, total, facturas_gastos)
```
